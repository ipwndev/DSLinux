...
int recognizer_unload(recognizer rec);
...
const rec_info *recognizer_get_info(recognizer rec);
...
int
recognizer_save_dictionary(recognizer rec, char *dir, char *name,
			   wordset dict);
...
int
recognizer_delete_from_dictionary(recognizer rec, letterset * word,
				  wordset dict);
...
int
recognizer_get_buffer(recognizer rec, u_int * nstrokes,
		      pen_stroke ** strokes);
...
char **recognizer_get_gesture_names(recognizer rec);
...
recognizer_set_gesture_action(recognizer rec,
			      char *name, xgesture fn, void *wsinof);
...
pen_stroke *copy_pen_stroke_array(u_int nstrokes, pen_stroke * strokes);
pen_state *copy_pen_state_array(u_int nstate, pen_state * state);
u_int *copy_state_trans_array(u_int ntrans, u_int * trans);
