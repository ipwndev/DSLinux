...
/* JHC:  Moved here from hre.h to avoid confusion */
...
recognizer_train(recognizer r, rc * rec_xt, u_int nstrokes,
		 pen_stroke * strokes, rec_element * re, bool replace_p)
{
    /*This operation isn't supported by the LI recognizer. */
...
li_recognizer_get_example(recognizer r,
			  int class,
			  int instance,
			  char **name, pen_point ** points, int *npts)
{
...
static int
li_recognizer_save(recognizer r, char *dir, char *filename)
{
    /*This operation isn't supported by the LI recognizer. */
...
li_recognizer_save_dictionary(recognizer rec,
			      char *directory, char *name, wordset dict)
{
    /*This operation isn't supported by the LI recognizer. */
...
static int
li_recognizer_add_to_dictionary(recognizer rec, letterset * word,
				wordset dict)
{
    /*This operation isn't supported by the LI recognizer. */
...
static int
li_recognizer_get_buffer(recognizer r, u_int * nstrokes,
			 pen_stroke ** strokes)
{
    /*This operation isn't supported by the LI recognizer. */
...
li_recognizer_set_gesture_action(recognizer r,
				 char *name, xgesture fn, void *wsinfo)
{
    /*This operation isn't supported by the LI recognizer. */
 ...
 */
static void
lialg_score_stroke(point_list * input_dompts, point_list * curr_dompts,
		   int *sim, int *dist)
{
    *sim = MIN_SIM;
    *dist = MAX_DIST;
...
lialg_canonicalize_examples(rClassifier * rec)
{
    ...
    for (nclasses = 0;
	 nclasses < MAXSCLASSES && rec->cnames[nclasses] != NULL; nclasses++);
	...
	for (j = 0, tmp = pts; j < nex; j++, tmp = tmp->next) {
		...
		if (lidebug) {
		    fprintf(stderr,
			    "lialg_canonicalize_example_stroke returned error\n");
		}
		return (-1);
	    }
	...
	for (j = 0, tmp = pts; j < nex; j++, tmp = tmp->next) {
	    int scalex =
		(tmp->xrange ==
		 0) ? 100 : (100 * maxxrange + tmp->xrange / 2) / tmp->xrange;
	    int scaley =
		(tmp->yrange ==
		 0) ? 100 : (100 * maxyrange + tmp->yrange / 2) / tmp->yrange;
	...
	if ((rec->canonex[i] =
	     add_example(NULL, avg->npts, avg->pts)) == NULL) {
	    delete_examples(pts);
	    return (-1);
	}
...
lialg_canonicalize_example_stroke(point_list * points)
{
	...
	if (lidebug) {
	    fprintf(stderr,
		    "lialg_translate_points (minx=%d,miny=%d,scale=%d) returned error\n",
		    minx, miny, scale);
	}
	return (-1);
    }
	...
	if (lidebug) {
	    fprintf(stderr,
		    "lialg_translate_points (minx=%d,miny=%d) returned error\n",
		    minx, miny);
	}
	return (-1);
    }
...
lialg_compute_equipoints(point_list * points)
{
    ...
    if (lidebug) {
	fprintf(stderr,
		"compute_equipoints:  npts = %d, pathlen = %d, equidist = %d\n",
...
lialg_translate_points(point_list * points,
		       int minx, int miny, int scalex, int scaley)
{
    int i;
