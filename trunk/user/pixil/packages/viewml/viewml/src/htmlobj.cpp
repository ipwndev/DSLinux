...
//const char *HTMLObject::getExec()
//{
//  printf("The actual function in HTMLObject\n");
//  return(exec);
//}
...
HTMLImage::HTMLImage(KHTMLWidget *widget,const char *_filename, char *_url,char *_target,char *_exec, int _max_width,int _width,int _height,int _percent,int bdr)
    : QObject(), HTMLObject()
#else
HTMLImage::HTMLImage(KHTMLWidget *widget,const char *_filename, char *_url,char *_target, int _max_width,int _width,int _height,int _percent,int bdr)
    : QObject(), HTMLObject()
#endif
{
    if ( pCache == 0 )
    ...
    // Commented out jsk .. we always want to load from the main thread ..
    // (plus the cache manager is unimplemented) .. please fix me
    //    if ( !kurl.isLocalFile() )
    if(1)
    {
      imageURL = _filename;
...
#ifdef EXEC_EXTENSIONS
HTMLImageMap::HTMLImageMap(KHTMLWidget *widget,const char *_filename,char *_url,char *_target,char *_exec,int _max_width,int _width,int _height,int _percent,int bdr)
  : HTMLImage(widget,_filename,_url,_target,_exec,_max_width,_width,_height,_percent,bdr)
#else
HTMLImageMap::HTMLImageMap(KHTMLWidget *widget,const char *_filename,char *_url,char *_target,int _max_width,int _width,int _height,int _percent,int bdr)
  : HTMLImage(widget,_filename,_url,_target,_max_width,_width,_height,_percent,bdr)
#endif
{
    type = ClientSide;
    serverurl = _url;
