  ...
  */
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    if (SyncCacheView(image_view[id]) == MagickFalse)
      status=MagickFalse;
    if (SetImageProgress(image,ClutImageTag,y,image->rows) == MagickFalse)
      status=MagickFalse;
  }
...
#define ContrastImageTag  "Contrast/Image"
  ...
  */
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    if (SyncCacheView(image_view[id]) == MagickFalse)
      status=MagickFalse;
    if (SetImageProgress(image,ContrastImageTag,y,image->rows) == MagickFalse)
      status=MagickFalse;
  }
  image_view=DestroyCacheViewThreadSet(image_view);
  return(status);
...
#define ContrastStretchImageTag  "ContrastStretch/Image"
  ...
  */
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    if (SyncCacheView(image_view[id]) == MagickFalse)
      status=MagickFalse;
    progress=SetImageProgress(image,ContrastStretchImageTag,y,image->rows);
  ...
  */
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    */
    ...
    if (SyncCacheView(enhance_view[id]) == MagickFalse)
      status=MagickFalse;
    if (SetImageProgress(image,EnhanceImageTag,y,image->rows) == MagickFalse)
      status=MagickFalse;
  }
  enhance_view=DestroyCacheViewThreadSet(enhance_view);
  enhance_view=DestroyCacheViewThreadSet(image_view);
...
#define EqualizeImageTag  "Equalize/Image"
  ...
  */
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    if (SyncCacheView(image_view[id]) == MagickFalse)
      status=MagickFalse;
    if (SetImageProgress(image,EqualizeImageTag,y,image->rows) == MagickFalse)
      status=MagickFalse;
  }
  image_view=DestroyCacheViewThreadSet(image_view);
  equalize_map=(MagickPixelPacket *) RelinquishMagickMemory(equalize_map);
  ...
  */
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    if (SyncImagePixels(image) == MagickFalse)
      status=MagickFalse;
    progress=SetImageProgress(image,GammaCorrectImageTag,y,image->rows);
    if (progress == MagickFalse)
      status=MagickFalse;
  }
  image_view=DestroyCacheViewThreadSet(image_view);
...
#define LevelValue(x) (RoundToQuantum((MagickRealType) QuantumRange* \
  ...
  */
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    if (SyncCacheView(image_view[id]) == MagickFalse)
      status=MagickFalse;
    if (SetImageProgress(image,LevelImageTag,y,image->rows) == MagickFalse)
      status=MagickFalse;
  }
  image_view=DestroyCacheViewThreadSet(image_view);
  return(status);
...
#define LevelizeValue(x) (RoundToQuantum(((MagickRealType) \
  ...
  */
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    if (SyncCacheView(image_view[id]) == MagickFalse)
      status=MagickFalse;
    if (SetImageProgress(image,LevelizeImageTag,y,image->rows) == MagickFalse)
      status=MagickFalse;
  }
  return(status);
}
...
*/
MagickBooleanType LevelImageColors(Image *image,const ChannelType channel,
     const MagickPixelPacket *black_color, const MagickPixelPacket
     *white_color, const MagickBooleanType invert)
{
...
#define ModulateImageTag  "Modulate/Image"
  ...
  */
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    if (SyncCacheView(image_view[id]) == MagickFalse)
      status=MagickFalse;
    if (SetImageProgress(image,ModulateImageTag,y,image->rows) == MagickFalse)
      status=MagickFalse;
  }
  image_view=DestroyCacheViewThreadSet(image_view);
  return(status);
  ...
  if (grayscale != MagickFalse)
    ...
    if (status == MagickFalse)
      continue;
        ...
        if (SyncCacheView(image_view[id]) == MagickFalse)
          status=MagickFalse;
        if (SetImageProgress(image,NegateImageTag,y,image->rows) == MagickFalse)
          status=MagickFalse;
      }
      image_view=DestroyCacheViewThreadSet(image_view);
      return(MagickTrue);
...
#define SigmoidalContrastImageTag  "SigmoidalContrast/Image"
  ...
  */
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    if (SyncCacheView(image_view[id]) == MagickFalse)
      status=MagickFalse;
    progress=SetImageProgress(image,SigmoidalContrastImageTag,y,image->rows);
    if (progress == MagickFalse)
      status=MagickFalse;
  }
  image_view=DestroyCacheViewThreadSet(image_view);
