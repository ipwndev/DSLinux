  ...
  const MetricType metric,double *distortion,ExceptionInfo *exception)
{
  ...
  */
  ...
  difference_view=AcquireCacheViewThreadSet(difference_image);
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    r=SetCacheViewPixels(difference_view[id],0,y,difference_image->columns,1);
    ...
    difference_indexes=GetCacheViewIndexes(difference_view[id]);
    ...
    for (x=0; x < (long) image->columns; x++)
    {
      ...
      if (difference != MagickFalse)
        MagickPixelCompositeOver(&source,7.5*QuantumRange/10.0,&highlight,
          (MagickRealType) highlight.opacity,&composite);
      else
        MagickPixelCompositeOver(&source,7.5*QuantumRange/10.0,&transparent,
          (MagickRealType) transparent.opacity,&composite);
      SetPixelPacket(difference_image,&composite,r,difference_indexes+x);
      p++;
      q++;
      r++;
    }
    if (SyncCacheView(difference_view[id]) == MagickFalse)
      status=MagickFalse;
  }
  difference_view=DestroyCacheViewThreadSet(difference_view);
  ...
  ExceptionInfo *exception)
{
  ...
  image_view=AcquireCacheViewThreadSet(image);
  reconstruct_view=AcquireCacheViewThreadSet(reconstruct_image);
  #pragma omp parallel for reduction(+ : distortion, area)
  for (y=0; y < (long) image->rows; y++)
  {
    register const IndexPacket
      *indexes,
