...
typedef struct {
	...
	unsigned int dir_size_computed:1; /* Size of directory is computed*/ /* --Hatred */
    } f;
    char *cache;
} file_entry;
...
void do_sort (dir_list *list, sortfn *sort, int top, int reverse, int case_sensitive, int exe_first);
dir_list *do_collect_stat (dir_list *dir, int top);
int do_reload_dir (dir_list *list, sortfn *sort, int count, int reverse, int case_sensitive, int exe_first, char *filter);
