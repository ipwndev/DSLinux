...
/* "$Id: dir.c,v 1.3 1998/04/03 20:00:06 unammx Exp $" */
...
add_dotdot_to_list (dir_list *list, int index)
{
    ...
    (list->list) [index].buf.st_size = 0;
    (list->list) [index].dir_size = 0; // WARNING!
    (list->list) [index].cache = NULL;
    (list->list) [index].f.link_to_dir = 0;
    (list->list) [index].f.stalled_link = 0;
    (list->list) [index].f.dir_size_computed = 0; /* Fix by Hatred */
...
int do_load_dir(dir_list *list, sortfn *sort, int reverse, int case_sensitive, int exe_first, char *filter)
{
    DIR           *dirp;
    struct dirent *dp;
	...
	list->list [next_free].f.dir_size_computed = 0; /* Fix by Hatred */
// WARNING!
//	list->list [next_free].dir_size = 0; // May I delete this? --Hatred // WARNING!
		   ...
		   int case_sensitive, int exe_first, char *filter)
{
	...
	dir_copy.list [i].f.dir_size_computed = list->list [i].f.dir_size_computed; /* Fix by Hatred */
	dir_copy.list [i].dir_size = list->list [i].dir_size; // WARNING!
    }
	...
	list->list [next_free].buf = buf;
	list->list [next_free].f.dir_size_computed = 0; /* Fix by Hatred */
	list->list [next_free].dir_size = 0; // WARNING! //--Olegarch
