...
struct mount_entry *read_filesystem_list (int need_fs_type, int all_fs)
{
	...
	while ((mnt = getmntent (fp))) {
	    ...
	    me = (struct mount_entry *) malloc (sizeof (struct mount_entry));
	    me->me_devname = (char * ) strdup (mnt->mnt_fsname);
	    me->me_mountdir = (char *) strdup (mnt->mnt_dir);
	    me->me_type = (char *) strdup (mnt->mnt_type);
	...
	while (entries-- > 0) {
	    me = (struct mount_entry *) malloc (sizeof (struct mount_entry));
	    me->me_devname = (char *) strdup (fsp->f_mntfromname);
	    me->me_mountdir = (char *) strdup (fsp->f_mntonname);
#ifdef xBSD
	    me->me_type = (char *) strdup (fsp->f_fstypename);
#else
