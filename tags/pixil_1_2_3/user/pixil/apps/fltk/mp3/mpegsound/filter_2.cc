...
#define OUT2(v)   out2[(96-(v)-32)*16]=out2[((v)-32)*16]
	...
	q8 = hcos_32[0] * (x[0] - x[15]);
	q9 = hcos_32[1] * (x[1] - x[14]);
	qa = hcos_32[2] * (x[2] - x[13]);
	qb = hcos_32[3] * (x[3] - x[12]);
	qc = hcos_32[4] * (x[4] - x[11]);
	qd = hcos_32[5] * (x[5] - x[10]);
	qe = hcos_32[6] * (x[6] - x[9]);
	qf = hcos_32[7] * (x[7] - x[8]);
    }
...
inline void
Mpegtoraw::generatesingle_2(void)
{
    int i;
    REAL r;
    register REAL *vp;
