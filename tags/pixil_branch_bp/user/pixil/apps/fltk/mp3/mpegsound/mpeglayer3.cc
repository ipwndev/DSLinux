...
//
// Fixed point support by Nicolas Pitre <nico@cam.org>
...
inline int
Mpegbitwindow::getbits9(int bits)
{
...
inline int
Mpegtoraw::wgetbit(void)
{
    return bitwindow.getbit();
}
inline int
Mpegtoraw::wgetbits9(int bits)
{
    return bitwindow.getbits9(bits);
}
inline int
Mpegtoraw::wgetbits(int bits)
{
    return bitwindow.getbits(bits);
}
...
inline void
long_memset(void *s, unsigned int c, int count)
{
...
#define TO_FOUR_THIRDS(x) \
	(((x) >= 0) ?  TO_FOUR_THIRDSTABLE[(x)] \
		    : -TO_FOUR_THIRDSTABLE[-(x)])
...
#include "mpeglayer3tables.h"


void
Mpegtoraw::layer3initialize(void)
{
    layer3framestart = 0;
    currentprevblock = 0;
...
bool
Mpegtoraw::layer3getsideinfo(void)
{
    sideinfo.main_data_begin = getbits(9);

    if (!inputstereo)
	sideinfo.private_bits = getbits(5);
    else
	sideinfo.private_bits = getbits(3);
	...
	for (ch = 0;; ch++) {
	    ...
	    gi->generalflag = gi->window_switching_flag
		&& (gi->block_type == 2);
...
bool
Mpegtoraw::layer3getsideinfo_2(void)
{
    sideinfo.main_data_begin = getbits(8);

    if (!inputstereo)
	sideinfo.private_bits = getbit();
    else
	sideinfo.private_bits = getbits(2);
...
void
Mpegtoraw::layer3getscalefactors(int ch, int gr)
{
    ...
    if (gi->generalflag) {
	if (gi->mixed_block_flag) {	/* MIXED *//* NEW-ag 11/25 */
	    sf->l[0] = wgetbits9(l0);
	    sf->l[1] = wgetbits9(l0);
	    sf->l[2] = wgetbits9(l0);
	    sf->l[3] = wgetbits9(l0);
	    ...
	    sf->s[0][3] = wgetbits9(l0);
	    sf->s[1][3] = wgetbits9(l0);
	    sf->s[2][3] = wgetbits9(l0);
	    sf->s[0][4] = wgetbits9(l0);
	    sf->s[1][4] = wgetbits9(l0);
	    sf->s[2][4] = wgetbits9(l0);
	    sf->s[0][5] = wgetbits9(l0);
	    sf->s[1][5] = wgetbits9(l0);
	    sf->s[2][5] = wgetbits9(l0);

	    sf->s[0][6] = wgetbits9(l1);
	    sf->s[1][6] = wgetbits9(l1);
	    sf->s[2][6] = wgetbits9(l1);
	    sf->s[0][7] = wgetbits9(l1);
	    sf->s[1][7] = wgetbits9(l1);
	    sf->s[2][7] = wgetbits9(l1);
	    sf->s[0][8] = wgetbits9(l1);
	    sf->s[1][8] = wgetbits9(l1);
	    sf->s[2][8] = wgetbits9(l1);
	    sf->s[0][9] = wgetbits9(l1);
	    sf->s[1][9] = wgetbits9(l1);
	    sf->s[2][9] = wgetbits9(l1);
	    sf->s[0][10] = wgetbits9(l1);
	    sf->s[1][10] = wgetbits9(l1);
	    sf->s[2][10] = wgetbits9(l1);
	    sf->s[0][11] = wgetbits9(l1);
	    sf->s[1][11] = wgetbits9(l1);
	    sf->s[2][11] = wgetbits9(l1);

	    sf->s[0][12] = sf->s[1][12] = sf->s[2][12] = 0;
	} else {		/* SHORT */
	    sf->s[0][0] = wgetbits9(l0);
	    sf->s[1][0] = wgetbits9(l0);
	    sf->s[2][0] = wgetbits9(l0);
	    sf->s[0][1] = wgetbits9(l0);
	    sf->s[1][1] = wgetbits9(l0);
	    sf->s[2][1] = wgetbits9(l0);
	    sf->s[0][2] = wgetbits9(l0);
	    sf->s[1][2] = wgetbits9(l0);
	...
	if (gr == 0) {
	    ...
	    sf->l[11] = wgetbits9(l1);
	    sf->l[12] = wgetbits9(l1);
	    sf->l[13] = wgetbits9(l1);
	    sf->l[14] = wgetbits9(l1);
	    sf->l[15] = wgetbits9(l1);
	    sf->l[16] = wgetbits9(l1);
	    sf->l[17] = wgetbits9(l1);
	    sf->l[18] = wgetbits9(l1);
	    sf->l[19] = wgetbits9(l1);
...
void
Mpegtoraw::layer3getscalefactors_2(int ch)
{
	...
	if (gi->block_type == 2)
	    blocktypenumber = 1 + gi->mixed_block_flag;
	else
	    blocktypenumber = 0;
