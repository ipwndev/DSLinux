...
//    hl = malloc (sizeof (struct hotlist));
    hl->type     = 0;
    hl->directory =
	hl->label = 0;
    hl->head =
...
update_path_name (void)
{
    ...
    p = copy_strings (" ", current_group->label, " ", NULL);
    if (!hotlist_state.moving)
...
//    char *hdr = (char *) strdup_printf (_("Moving %s"), item->label);
...
{
    return  copy_strings (grp->directory, ".Group", NULL);
...
add2hotlist (char *label, char *directory, enum HotListType type, int pos)
{
	...
	if (type == HL_TYPE_GROUP) {
	    char  *lbl = copy_strings ("->", new->label, NULL);
...
#define RELATIVE_Y_INPUT_PTH	4
    ...
    { quick_button, 55, 80, RELATIVE_Y_BUTTONS, 0, N_("&Cancel"), 0, B_CANCEL, 0, 0, XV_WLAY_DONTCARE, "button-cancel" },
    { quick_button, 30, 80, RELATIVE_Y_BUTTONS, 0, N_("&Insert"), 0, B_INSERT, 0, 0, XV_WLAY_DONTCARE, "button-insert" },
    { quick_button, 10, 80, RELATIVE_Y_BUTTONS, 0, N_("&Append"), 0, B_APPEND, 0, 0, XV_WLAY_DONTCARE, "button-append" },
    { quick_input,  4, 80, RELATIVE_Y_INPUT_PTH, 0, "",58, 0, 0, 0, XV_WLAY_BELOWCLOSE, "input-pth" },
    { quick_label, RELATIVE_Y_LABEL_PTH, 80, 3, 0, 0, 0, 0, 0, 0, XV_WLAY_DONTCARE, "label-pth" },
...
static void add_new_entry_cmd (void)
{
    ...
    ret = add_new_entry_input (_("New hotlist entry"), _("Directory label"), _("Directory path"), "[Hotlist]", &title, &url);

    if (!ret || !title || !*title || !url || !*url)
	return;

    if (ret == B_ENTER || ret == B_APPEND)
	add2hotlist ((char *) strdup (title),(char *) strdup (url), HL_TYPE_ENTRY, 2);
    else
	add2hotlist ((char *) strdup (title),(char *) strdup (url), HL_TYPE_ENTRY, 1);
...
static int add_new_group_input (char *header, char *label, char **result)
{
    ...
    QuickDialog Quick_input;
    static QuickWidget quick_widgets [] = {
    { quick_button, 55, 80, 1, 0, N_("&Cancel"), 0, B_CANCEL, 0, 0, XV_WLAY_DONTCARE, "button-cancel" },
    { quick_button, 30, 80, 1, 0, N_("&Insert"), 0, B_INSERT, 0, 0, XV_WLAY_DONTCARE, "button-insert" },
    { quick_button, 10, 80, 1, 0, N_("&Append"), 0, B_APPEND, 0, 0, XV_WLAY_DONTCARE, "button-append" },
...
void add2hotlist_cmd (void)
{
    ...
    add2hotlist (label,(char *) strdup (cpanel->cwd), HL_TYPE_ENTRY, 0);
    hotlist_state.modified = 1;
}
...
load_group (struct hotlist *grp)
{
	...
	add2hotlist ((char *) strdup (value), (char *) strdup (key), HL_TYPE_GROUP, 0);
    }
    free (group_section);
    ...
    while (profile_keys){
	profile_keys = profile_iterator_next (profile_keys, &key, &value);
	add2hotlist ((char *) strdup (value),(char *) strdup (key), HL_TYPE_ENTRY, 0);
    }
...
hot_load_group (struct hotlist * grp)
{
	...
	case TKN_GROUP:
	    ...
	    new_grp = add2hotlist ((char *) strdup (tkn_buf), 0, HL_TYPE_GROUP, 0);
	    SKIP_TO_EOL;
	...
	case TKN_ENTRY:
	    CHECK_TOKEN(TKN_STRING);
	    label = (char *) strdup (tkn_buf);
	    CHECK_TOKEN(TKN_URL);
	    CHECK_TOKEN(TKN_STRING);
	    url = (char *) strdup (tkn_buf);
...
clean_up_hotlist_groups (char *section)
{
    ...
    grp_section = copy_strings (section, ".Group", NULL);
...
void load_hotlist (void)
{
    ...
    hotlist->label     = (char *) strdup (_(" Top level group "));
    hotlist->up        = hotlist;
    /*
     * compatibility :-(
     */
    hotlist->directory = (char *) strdup ("Hotlist");
...
int save_hotlist (void)
{
	...
	char	*fbak = copy_strings (hotlist_file_name, ".bak", NULL);
