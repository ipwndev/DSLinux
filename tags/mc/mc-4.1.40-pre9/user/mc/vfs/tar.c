...
void tarfs_fill_names (void (*func)(char *))
{
	...
	name = copy_strings (a->name, "#tar", PATH_SEP_STR,
			     a->current_dir->name, 0);
	(*func)(name);
	free (name);
	a = a->next;
					...
					int size, int fd, int type)
{
    ...
    tmp_handle = mc_mkstemp (&(current_archive->tmpname), "tar-", NULL);
...
static int read_header (struct tarfs_archive *archive, int tard)
{
  ...
  recurse:
	...
	if (current_file_name[len - 1] == PATH_SEP) {
	...
	if (len && current_link_name [len - 1] == PATH_SEP)
...
/* Add. --Hatred */
char  tarfs_is_tar(char *name)
{
	char *p;
	for(p=name+strlen(name);p>=name;p--)
	{                
		if( *p=='#' && (p+1) && strlen(p+1)>=3 && !strncmp(p+1,"tar",3))
			return 1; 
	}
...
for(p=name+strlen(name);p>=name;p--)
{
	...
	if(*p=='#' && strlen(p+1)>=3 && !strncmp(p+1,"tar",3))
	{
		if(p+4)
		{
			local = strdup(p+4);	
