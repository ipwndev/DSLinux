...
/* "$Id: setup.c,v 1.9 1998/05/12 04:26:32 unammx Exp $" */
...
static struct {
    ...
    { "show_mountpoint_info", &show_mountpoint_info },
    { "xterm_title", &xterm_title },
    { "clock_in_editor", &clock_in_editor },
...
// ---- double frames
    ...
    { "panel_scrollbar", &panel_scrollbar },
// ---- mountpoints menu option
    { "mc_cd_mountpoint_and_dir", &mc_cd_mountpoint_and_dir },
    { 0, 0 }
};
...
static struct {
    ...
    { "editor_edit_save_position", &option_edit_save_pos },
#endif
...
void panel_save_setup (WPanel *panel, char *section)
{
    ...
    sprintf (buffer, "%d", panel->exe_first);
...
/*     directory_history_save (); */
    ...
    save_string ("File Types", "colorize_executables", colorize_executables ? "1" : "0", profile_name);
    save_string ("File Types", "temp", ftmp, profile_name);
    save_string ("File Types", "doc", fdoc, profile_name);
    save_string ("File Types", "archive", farch, profile_name);
    save_string ("File Types", "source", fsrc, profile_name);
    save_string ("File Types", "media", fmedia, profile_name);
    save_string ("File Types", "graphics", fgraph, profile_name);
    save_string ("File Types", "database", fdbase, profile_name);

#ifdef HAVE_CHARSETS
     save_string( "Misc", "display_codepage",
    		 (char *) get_codepage_filename( display_codepage ), profile_name );
#endif /* HAVE_CHARSETS */
...
void load_setup (void)
{
	...
	*options [i].opt_addr =
    ...
    colorize_executables = load_int ("File Types", "colorize_executables", 1);
    ftmp   = do_load_string ("File Types", "temp",
                                "tmp,$$$,~,bak");
    fdoc   = do_load_string ("File Types", "doc",
                                "txt,doc,rtf,diz,ctl,me,ps,pdf,xml,xsd,xslt,dtd,html,shtml,htm,mail,msg,lsm,po,nroff,man,tex,sgml,css,text,letter");
    farch  = do_load_string ("File Types", "archive",
                                "gz,bz2,tar,tgz,rpm,Z,rar,zip,arj,cab,lzh,lha,zoo,arc,ark");
    fsrc   = do_load_string ("File Types", "source",
...
static int lookup_key_with_modspec( char * keyspec ) {
    if ((strlen(keyspec)<3) || (keyspec[1]!='-') ) {
	return lookup_key( keyspec );
    }
    switch( keyspec[0] ) {
	case 'C': return KEY_M_CTRL | lookup_key_with_modspec( keyspec + 2 );
	case 'S': return KEY_M_SHIFT | lookup_key_with_modspec( keyspec + 2 );
	case 'A': return KEY_M_ALT | lookup_key_with_modspec( keyspec + 2 );
    }
    return lookup_key( keyspec );
}

void load_keys_from_section (char *terminal, char *profile_name, int max_recurse)
{
    char *section_name;
    void *profile_keys;
	...
	if (!strcmp(key,"include")) {
	    ...
	    load_keys_from_section( valcopy, profile_name , max_recurse - 1 );
	} else {
	    key_code = lookup_key_with_modspec (key);
...
void load_key_defs (void)
{
    ...
    load_keys_from_section (getenv ("TERM"), profile_name,10);
    load_keys_from_section ("general", profile_name,10);

    load_keys_from_section (getenv ("TERM"), libfile, 10);
    load_keys_from_section ("general", libfile, 10);
