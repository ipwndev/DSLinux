...
/* "$Id: util.c,v 1.6 1998/04/08 19:37:08 pavel Exp $" */
...
char *name_trunc (char *txt, int trunc_len)
{
	...
	fprintf (stderr, _("name_trunc: too big, text:%s, trunc size:%d"), txt, trunc_len);
...
char *size_trunc (long long size){
    static char x [30];
#if 0
    long long divisor = 1;
    char *xtra = "";
    ...
    sprintf (x, "%lld%s", (size/divisor), xtra);
#else
    sprintf (x, "%lld", size);
#endif
    return x;
...
char *size_trunc_sep (long long size)
{
    static char x [60];
    int  count;
    char *p, *d, *y;
...
char *unix_error_string (int error_num)
{
  ...
  if ((error_num > 0) && (errnum <= sys_nerr))
...
mc_tmpdir (void)
{
	...
	tmpdir = sys_tmp;
	test_fd = mc_mkstemp (&test_fn, "mctest", NULL);
	if (test_fd != -1) {
	    close (test_fd);
...
int
mc_mkstemp (char **pname, const char *prefix, const char *suffix)
{
	...
	tmpbase = strdup (prefix);
    }

    tmpname = copy_strings (tmpbase, "XXXXXX", suffix, NULL);
    *pname = tmpname;
