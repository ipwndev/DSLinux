...
#define AddNoiseImageTag  "AddNoise/Image"
  ...
  */
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    {
      if ((channel & RedChannel) != 0)
        (q+x)->red=GenerateNoise((p+x)->red,noise_type,attenuate);
      if ((channel & GreenChannel) != 0)
        (q+x)->green=GenerateNoise((p+x)->green,noise_type,attenuate);
      if ((channel & BlueChannel) != 0)
        (q+x)->blue=GenerateNoise((p+x)->blue,noise_type,attenuate);
      if ((channel & OpacityChannel) != 0)
        (q+x)->opacity=GenerateNoise((p+x)->opacity,noise_type,attenuate);
      if (((channel & IndexChannel) != 0) &&
          (image->colorspace == CMYKColorspace))
...
#define BlurImageTag  "Blur/Image"
  ...
  */
  ...
  blur_view=AcquireCacheViewThreadSet(blur_image);
  ...
  for (y=0; y < (long) blur_image->rows; y++)
  {
    ...
    p=AcquireCacheViewPixels(image_view[id],-((long) width/2L),y,image->columns+
      width,1,exception);
    q=GetCacheViewPixels(blur_view[id],0,y,blur_image->columns,1);
    ...
    blur_indexes=GetCacheViewIndexes(blur_view[id]);
    for (x=0; x < (long) blur_image->columns; x++)
    {
      MagickPixelPacket
        pixel;
    ...
    if (SyncCacheView(blur_view[id]) == MagickFalse)
      status=MagickFalse;
    progress=SetImageProgress(image,BlurImageTag,y,blur_image->rows+
      blur_image->columns);
  ...
  */
  ...
  image_view=AcquireCacheViewThreadSet(blur_image);
  for (x=0; x < (long) blur_image->columns; x++)
  {
    register const IndexPacket
      *indexes;
    ...
    if (status == MagickFalse)
      continue;
    id=GetCacheViewThreadId();
    p=AcquireCacheViewPixels(image_view[id],x,-((long) width/2L),1,image->rows+
      width,exception);
    q=GetCacheViewPixels(blur_view[id],x,0,1,blur_image->rows);
    if ((p == (const PixelPacket *) NULL) || (q == (PixelPacket *) NULL))
      {
        status=MagickFalse;
        continue;
    ...
    if (SyncCacheView(blur_view[id]) == MagickFalse)
      status=MagickFalse;
    progress=SetImageProgress(image,BlurImageTag,(MagickOffsetType) image->rows+
      x,image->rows+image->columns);
    if (progress == MagickFalse)
      x=(long) blur_image->columns;
  }
  blur_image->type=image->type;
  blur_view=DestroyCacheViewThreadSet(blur_view);
  image_view=DestroyCacheViewThreadSet(image_view);
  kernel=(double *) RelinquishMagickMemory(kernel);
  if (status == MagickFalse)
    blur_image=DestroyImage(blur_image);
...
static MedianPixelList *RelinquishMedianPixelList(MedianPixelList *pixel_list)
{
  register long
    i;
...
static MedianPixelList *AcquireMedianPixelList(unsigned long width)
{
  MedianPixelList
    *pixel_list;
  ...
  for (i=0; i < MedianListChannels; i++)
  {
    ...
    if (pixel_list->lists[i].nodes == (MedianListNode *) NULL)
      return(RelinquishMedianPixelList(pixel_list));
  }
  pixel_list->signature=MagickSignature;
  return(pixel_list);
}

static void AddNodeMedianPixelList(MedianPixelList *pixel_list,long channel,
  unsigned long color)
{
  register long
    level;
...
#define MedianFilterImageTag  "MedianFilter/Image"
  ...
  */
  pixel_list=AcquireMedianPixelList(width);
  if (pixel_list == (MedianPixelList *) NULL)
    {
      pixel_list=RelinquishMedianPixelList(pixel_list);
      median_image=DestroyImage(median_image);
      ThrowImageException(ResourceLimitError,"MemoryAllocationFailed");
    }
  for (y=0; y < (long) median_image->rows; y++)
  {
    ...
    for (x=0; x < (long) median_image->columns; x++)
    {
      r=p;
      s=indexes+x;
      ResetMedianPixelList(pixel_list);
      for (v=0; v < (long) width; v++)
      {
        for (u=0; u < (long) width; u++)
          InsertMedianPixelList(image,r+u,s+u,pixel_list);
        r+=image->columns+width;
        s+=image->columns+width;
      }
      pixel=GetMedianPixelList(pixel_list);
  ...
  const double sigma,const double angle,ExceptionInfo *exception)
{
  ...
  */
  ...
  offset.x=(double) width*sin(DegreesToRadians(angle));
  offset.y=(double) width*cos(DegreesToRadians(angle));
  for (i=0; i < (long) width; i++)
  {
    offsets[i].x=(double) (i*offset.y)/hypot(offset.x,offset.y);
    offsets[i].y=(double) (i*offset.x)/hypot(offset.x,offset.y);
  }
  alpha=1.0;
  for (y=0; y < (long) image->rows; y++)
  {
  ...
  */
  ...
  for (y=0; y < (long) noise_image->rows; y++)
  {
    ...
    for (x=0; x < (long) noise_image->columns; x++)
    {
      ...
      pixel=GetNonpeakMedianPixelList(pixel_list);
...
#define SpreadImageTag  "Spread/Image"
  ...
  */
  width=GetOptimalKernelWidth1D(radius,0.5);
  GetMagickPixelPacket(spread_image,&pixel);
  resample_filter=AcquireResampleFilter(image,exception);
  spread_view=AcquireCacheView(spread_image);
  for (y=0; y < (long) spread_image->rows; y++)
  {
    q=SetCacheViewPixels(spread_view,0,y,spread_image->columns,1);
    if (q == (PixelPacket *) NULL)
      break;
    indexes=GetCacheViewIndexes(spread_view);
    for (x=0; x < (long) spread_image->columns; x++)
    {
      pixel=ResamplePixelColor(resample_filter,(double) x+width*
        (GetPseudoRandomValue()-0.5),(double) y+width*
        (GetPseudoRandomValue()-0.5));
      SetPixelPacket(spread_image,&pixel,q,indexes+x);
      q++;
    }
    if (SyncCacheView(spread_view) == MagickFalse)
      break;
    if ((image->progress_monitor != (MagickProgressMonitor) NULL) &&
        (QuantumTick(y,image->rows) != MagickFalse))
      {
...
#define SharpenImageTag  "Sharpen/Image"
  ...
  status=MagickTrue;
  image_view=AcquireCacheViewThreadSet(image);
  unsharp_view=AcquireCacheViewThreadSet(unsharp_image);
  #pragma omp parallel for
  for (y=0; y < (long) image->rows; y++)
  {
    const IndexPacket
    ...
    q=GetCacheViewPixels(unsharp_view[id],0,y,unsharp_image->columns,1);
    if ((p == (const PixelPacket *) NULL) || (q == (PixelPacket *) NULL))
      {
        status=MagickFalse;
        continue;
    ...
    indexes=AcquireCacheViewIndexes(image_view[id]);
    unsharp_indexes=GetCacheViewIndexes(unsharp_view[id]);
    for (x=0; x < (long) image->columns; x++)
    {
      MagickPixelPacket
        pixel;
    ...
    if (SyncCacheView(unsharp_view[id]) == MagickFalse)
      status=MagickFalse;
    if (SetImageProgress(image,SharpenImageTag,y,image->rows) == MagickFalse)
      status=MagickFalse;
  }
  unsharp_image->type=image->type;
  unsharp_view=DestroyCacheViewThreadSet(unsharp_view);
  image_view=DestroyCacheViewThreadSet(image_view);
  if (status == MagickFalse)
