  ...
  const CubeInfo *cube_info,const RealPixelPacket *pixel,unsigned long index)
{
  ...
  id=(unsigned long) (
    ((ScaleQuantumToChar(ClipToQuantum(pixel->red)) >> index) & 0x01) |
    ((ScaleQuantumToChar(ClipToQuantum(pixel->green)) >> index) & 0x01) << 1 |
    ((ScaleQuantumToChar(ClipToQuantum(pixel->blue)) >> index) & 0x01) << 2);
  if (IsAssociatedAlpha(image,cube_info) != MagickFalse)
    id|=((ScaleQuantumToChar(ClipToQuantum(pixel->opacity)) >> index) & 0x01)
      << 3;
  return(id);
}
  ...
  else
    {
      ...
      for (y=0; y < (long) image->rows; y++)
      {
        ...
        for (x=0; x < (long) image->columns; x+=count)
        {
          ...
          */
          ...
          cube_info->distance=4.0*((MagickRealType) QuantumRange+1.0)*
            ((MagickRealType) QuantumRange+1.0)+1.0;
        ...
        if (SyncCacheView(image_view) == MagickFalse)
          break;
        if (SetImageProgress(image,AssignImageTag,y,image->rows) == MagickFalse)
          break;
      }
...
#define ClassifyImageTag  "Classify/Image"
  ...
  */
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    if (SetImageProgress(image,ClassifyImageTag,y,image->rows) == MagickFalse)
      break;
  ...
  if (y == (long) image->rows)
    cube_info->quantize_info->dither=MagickFalse;
  ...
  if ((p->x >= 0) && (p->x < (long) image->columns) &&
      (p->y >= 0) && (p->y < (long) image->rows))
    {
      ...
      */
      ...
      progress=SetImageProgress(image,DitherImageTag,p->offset,p->span);
      p->offset++;
    }
  switch (direction)
  {
  ...
  Image *images)
{
  ...
  */
  ...
  for (i=0; image != (Image *) NULL; i++)
  {
    ...
    if (SetImageProgress(image,AssignImageTag,i,number_images) == MagickFalse)
      break;
      ...
      */
      ...
      for (i=0; image != (Image *) NULL; i++)
      {
        ...
        (void) SetImageProgressMonitor(image,progress_monitor,
          image->client_data);
        progress=SetImageProgress(image,AssignImageTag,i,number_images);
...
#define ReduceImageTag  "Reduce/Image"
  ...
  while (cube_info->colors > cube_info->maximum_colors)
  {
    ...
    progress=SetImageProgress(image,ReduceImageTag,offset,span-
...
static MagickBooleanType SetGrayscaleImage(Image *image)
{
  ...
  colormap_index=(long *) AcquireQuantumMemory(MaxColormapSize,
  ...
  if (image->storage_class != PseudoClass)
    {
      for (i=0; i < (long) MaxColormapSize; i++)
        colormap_index[i]=(-1);
  ...
  for (i=0; i < (long) image->colors; i++)
  {
    if (IsColorEqual(&colormap[j],&image->colormap[i]) == MagickFalse)
      {
        j++;
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    indexes=GetIndexes(image);
    for (x=0; x < (long) image->columns; x++)
      indexes[x]=(IndexPacket) colormap_index[indexes[x]];
    if (SyncImagePixels(image) == MagickFalse)
      break;
  }
  colormap_index=RelinquishMagickMemory(colormap_index);
