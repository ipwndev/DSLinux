...
static ContributionInfo **DestroyThreadedContribution(
...
static ContributionInfo **AcquireThreadedContribution(const size_t count)
{
  register long
    i;
  ...
  for (i=0; i < (long) GetCacheViewMaximumThreads(); i++)
  {
    contribution[i]=AcquireQuantumMemory(count,sizeof(**contribution));
    if (contribution[i] == (ContributionInfo *) NULL)
      return(DestroyThreadedContribution(contribution));
  }
  return(contribution);
}
...
#define ResizeImageTag  "Resize/Image"
  ...
  */
  ...
  contribution=AcquireThreadedContribution((size_t) (2.0*support+3.0));
  if (contribution == (ContributionInfo **) NULL)
    {
  ...
  for (x=0; x < (long) resize_image->columns; x++)
  {
    ...
    for (y=0; y < (long) resize_image->rows; y++)
    {
      ...
      if (image->matte == MagickFalse)
      ...
      else
        {
          ...
          (q+y)->red=RoundToQuantum(gamma*pixel.red);
          (q+y)->green=RoundToQuantum(gamma*pixel.green);
          (q+y)->blue=RoundToQuantum(gamma*pixel.blue);
          (q+y)->opacity=RoundToQuantum(pixel.opacity);
        }
      if ((image->colorspace == CMYKColorspace) &&
    ...
    if (SyncCacheView(resize_view[id]) == MagickFalse)
      status=MagickFalse;
    if (SetImageProgress(image,ResizeImageTag,(*quantum)++,span) == MagickFalse)
      x=(long) resize_image->columns;
  }
  resize_view=DestroyCacheViewThreadSet(resize_view);
  image_view=DestroyCacheViewThreadSet(image_view);
  contribution=DestroyThreadedContribution(contribution);
  return(status);
}
  ...
  const MagickSizeType span,MagickOffsetType *quantum,ExceptionInfo *exception)
{
  ...
  */
  ...
  for (y=0; y < (long) resize_image->rows; y++)
  {
    ...
    if (SyncCacheView(resize_view[id]) == MagickFalse)
      status=MagickFalse;
    progress=SetImageProgress(image,ResizeImageTag,(*quantum)++,span);
  ...
  const unsigned long columns,const unsigned long rows,ExceptionInfo *exception)
{
  ...
  if (thumbnail_image->matte == MagickFalse)
    (void) SetImageAlphaChannel(thumbnail_image,ResetAlphaChannel);
