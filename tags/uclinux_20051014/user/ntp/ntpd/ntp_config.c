	...
	)
{
	...
	for (;;) {
		...
		switch(tok) {
			...
			/*
			 *  ntpd forks and calls ntp_intres(), resolves the names
			 *  and then sends NTP configuration packets to the parent.
			 *  This won't work on uClinux, which only has vfork(),
			 *  and vfork() will suspend execution of the parent until
			 *  exit/exec of the child.
			 *
			 *  So, we just give gethostbyname() a go.
			...
			if (!getnetnum(tokens[1], &peeraddr, 0)) {
				...
				hp = gethostbyname(tokens[1]);
				if (hp && hp->h_addrtype == AF_INET) {
					peeraddr.sin_family = AF_INET;
					peeraddr.sin_port = htons(NTP_PORT);
					memcpy(&peeraddr.sin_addr.s_addr, hp->h_addr, sizeof(struct in_addr) );
				}
			}
...
#ifdef __uClinux__
				fprintf(stderr, "uClinux: '%s' is not an IP address nor resolvable by gethostbyname()!\n", tokens[1]);
				fprintf(stderr, "uClinux: save_resolv()/init_res() don't work, uses fork()\n");
#else
