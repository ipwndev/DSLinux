...
KHTMLWidget::KHTMLWidget(QWidget *parent,const char *name,const char *)
  : KDNDWidget(parent,name,WPaintClever),tempStrings(true),parsedURLs(false),parsedTargets(false),parsedExecs(false)
#else
KHTMLWidget::KHTMLWidget(QWidget *parent,const char *name,const char *)
  : KDNDWidget(parent,name,WPaintClever),tempStrings(true),parsedURLs(false),parsedTargets(false)
#endif
{
    jsEnvironment = 0;      
...
//  else
//  {
//	  cerr << "There are " << mapPendingFiles.count() << " remaining on queue.\n";
//  } // end of else
...
#ifdef EXEC_EXTENSIONS
    if( !pressedExec.isEmpty())
    {
      if(_mouse->button() != RightButton)
	emit execfunc(pressedExec.data());
    }
#endif
...
{
  printf("Inside KHTMLWidget::getURL()\n");
...
void KHTMLWidget::paintEvent( QPaintEvent* _pe )
{
    ...
    if (bIsSelected && htmlView && htmlView->getFrameBorder())
    {
      
      cerr << "Drawing frame (frameborder = " << htmlView->getFrameBorder() << " )\n";
...
#ifdef EXEC_EXTENSIONS
    for ( str = parsedExecs.first(); str; str = parsedExecs.next() ) {
      delete [] str;
    }
    parsedExecs.clear();
#endif
...
/*
    ...
    while(parseBody(clue,__end,false));
*/
}
...
void KHTMLWidget::insertText(char *str, const HTMLFont * fp)
{
    ...
    for(;;)
    {
        ...
        if (insertBlock)
        {
...
#ifdef EXEC_EXTENSIONS
		  if(url || target || exec)
		    flow->append(new HTMLLinkTextMaster(str,fp,painter,url,target,exec));
#else
		  if(url || target)
		    flow->append(new HTMLLinkTextMaster(str,fp,painter,url,target));
#endif
	            else
...
#ifdef EXEC_EXTENSIONS
		  if(url || target || exec)
		    flow->append(new HTMLLinkText(str,fp,painter,url,target,exec));
#else
		  if(url || target )
		    flow->append(new HTMLLinkText(str,fp,painter,url,target));
#endif
		    else
	      ...
	      if(url || target || exec)
              {
		HTMLLinkText *sp = new HTMLLinkText(" ",fp,painter,url,target,exec);
#else
	      if(url || target)
              {
		HTMLLinkText *sp = new HTMLLinkText(" ",fp,painter,url,target);
#endif
...
//	vspace_inserted = insertVSpace( _clue, vspace_inserted );
...
#ifdef EXEC_EXTENSIONS
	    else if(strncasecmp(p,"exec=",5) == 0)
	    {
	      tmpexec = p + 5;
	    }
#endif
	...
	{
	  vspace_inserted = false;
	  colorStack.push(new QColor(settings->eLinkColor));
	  if(settings->underlineLinks)
	    underline = true;
	  selectFont();
	  exec = new char [tmpexec.length() + 1];
	  strcpy(exec,tmpexec.data());
	  parsedExecs.append(exec);
	}
#endif
	  ...
	  if ( !htmlView )
	      ...
	      while ( stringTok->hasMoreTokens() )
		{
		  ...
		  if ( strncasecmp( token, "SRC=", 4 ) == 0 )
		    ...
		    {
		      if ( strncasecmp( token + 12, "yes", 3 ) == 0 )
			frameborder = 1;
		      else if ( strncasecmp( token + 12, "no", 2 ) == 0 )
			frameborder = 0;
		      else 
		...
		p = framesetStack.getLast();
		  ...
		  if ( top )
		  {
		    ...
		    // this is a hack .. how do I get the documentrequests for 
		    // frame objects without it? .. jsk
		    connect( html, SIGNAL( documentRequest( KHTMLView *,const char* ) ),
			     top, SLOT( slotDocumentRequest( KHTMLView *,const char * ) ) );
		    connect( html, SIGNAL( imageRequest( KHTMLView *,const char* ) ),
			     top, SLOT( slotImageRequest( KHTMLView *,const char * ) ) );
		    connect( html, SIGNAL( URLSelected( KHTMLView *,const char*,int,const char* ) ),
			     top, SLOT( slotURLSelected( KHTMLView *,const char *,int,const char* )));
		    // end hack
...
void KHTMLWidget::parseI( HTMLClueV *_clue, const char *str )
{
	...
	if ( filename != 0 )
	{
	    ...
	    {
#ifdef EXEC_EXTENSIONS
	      image =  new HTMLImage(this,kurl.url(),url,target,exec,_clue->getMaxWidth(),width,height,percent,border);
#else
	      image =  new HTMLImage(this,kurl.url(),url,target,_clue->getMaxWidth(),width,height,percent,border);
#endif
	    }
	    else
	    {
...
#ifdef EXEC_EXTENSIONS
		image = new HTMLImageMap(this,kurl.url(),url,target,exec,_clue->getMaxWidth(),width,height,percent,border);
#else
		image = new HTMLImageMap(this,kurl.url(),url,target,_clue->getMaxWidth(),width,height,percent,border);
#endif
		if ( !usemap.isEmpty() )
...
// CRH	    if ( handlers == 0 )
// CRH	    {
// CRH		handlers = new QList<JSEventHandler>;
// CRH		handlers->setAutoDelete( TRUE );
// CRH	    }
// CRH	    handlers->append( new JSEventHandler( getJSEnvironment(), "onClick", code ) );
	}     
    }
...
KHTMLWidget::restore(SavedPage *p)
{
	...
	htmlView->openURL( ("restored:" + p->url).c_str() );
