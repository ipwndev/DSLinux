  ...
  for (scene=0; scene < (long) fits_info.number_planes; scene++)
  {
    ...
    if ((fits_info.min_data != 0.0) || (fits_info.max_data != 0.0))
      {
        if ((fits_info.bits_per_pixel != 0) && (fits_info.max_data == 0.0))
          fits_info.max_data=GetFITSQuantumRange(1UL*
    ...
    */
    ...
    if ((fits_info.bits_per_pixel < 0) || ((fits_info.max_data-
        fits_info.min_data) > GetFITSQuantumRange(image->depth)))
      scale=GetFITSQuantumRange(image->depth)/(fits_info.max_data-
        fits_info.min_data);
    p=fits_pixels;
    for (y=(long) image->rows-1; y >= 0; y--)
    {
      ...
      for (x=0; x < (long) image->columns; x++)
      {
        ...
        scale_pixel=scale*(pixel*fits_info.scale-fits_info.min_data+
          fits_info.zero);
        index=(IndexPacket) ((scale_pixel < 0.0) ? 0 : (scale_pixel >
          GetFITSQuantumRange(image->depth)) ? (unsigned long)
          GetQuantumRange(image->depth) : (unsigned long) (scale_pixel+0.5));
  ...
  for (i=0; i < (long) number_planes; i++)
  {
    for (y=(long) image->rows-1; y >= 0; y--)
    {
      ...
      for (x=0; x < (long) image->columns; x++)
      {
        ...
        if (image->depth > 8)
          pixel+=1.0*((MagickOffsetType) GetQuantumRange(image->depth));
        if (image->depth > 16)
          {
