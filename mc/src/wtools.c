...
/*  [] = "$Id: wtools.c,v 1.10 1998/05/19 18:37:08 unammx Exp $" */
...
extern void common_dialog_repaint_local (struct Dlg_head *h);
...
// set dialog colors
    ...
    if (flags & D_ERROR) {
	for ( i=0; i<4; i++ )
	query_colors[i] = alarm_colors[i];
    } else {
	for ( i=0; i<4; i++ )
	query_colors[i] = dialog_colors[i];
    }
...
// The data we need to pass to the callback
	...
	cols = (query_dlg->cols - win_len - 2) / 2 + 2;
	va_start (ap, count);
	for (i = 0; i < count; i++){
...
int quick_dialog_skip (QuickDialog *qd, int nskip)
{
	...
	case quick_input:
	    input = input_new (ypos, xpos, INPUT_COLOR_DEF,
...
/* we need a unique name for tkname because widget.c:history_tool()
   needs a unique name for each dialog - using the header is ideal */
    ...
    if (strncmp (header, _(" Search "), strlen (_(" Search "))-1) == 0){
        strncpy (tk_name, "search-string", 60);
    } else {
