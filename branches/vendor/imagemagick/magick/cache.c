...
static inline const PixelPacket *AcquireCacheVirtualNexus(const Image *image,
  ...
  const unsigned long rows,const unsigned long nexus,ExceptionInfo *exception)
{
  ...
  */
  ...
  if (IsNexusInCore(cache_info,nexus) != MagickFalse)
    return(pixels);
  if (ReadCachePixels(cache_info,nexus,exception) == MagickFalse)
    return((const PixelPacket *) NULL);
  ...
  const unsigned long nexus,ExceptionInfo *exception)
{
  ...
  */
  ...
  virtual_nexus=GetNexus(cache_info);
  if (virtual_nexus == 0)
    {
  ...
  for (v=0; v < (long) rows; v++)
  {
    for (u=0; u < (long) columns; u+=length)
    {
          ...
          (((y+v) < 0) || ((y+v) >= (long) cache_info->rows)) || (length == 0))
        {
          ...
          */
          ...
          switch (virtual_pixel_method)
          {
            ...
            {
              p=AcquireCacheVirtualNexus(image,EdgeX(cache_info->columns,x+u),
                EdgeY(cache_info->rows,y+v),1UL,1UL,virtual_nexus,exception);
            ...
            {
              p=AcquireCacheVirtualNexus(image,DitherX(cache_info->columns,x+u),
                DitherY(cache_info->rows,y+v),1UL,1UL,virtual_nexus,exception);
              break;
            }
            ...
            {
              p=AcquireCacheVirtualNexus(image,MirrorX(cache_info->columns,x+u),
                MirrorY(cache_info->rows,y+v),1UL,1UL,virtual_nexus,exception);
              break;
            }
            case RandomVirtualPixelMethod:
            {
              p=AcquireCacheVirtualNexus(image,RandomX(cache_info->columns),
                RandomY(cache_info->rows),1UL,1UL,virtual_nexus,exception);
              break;
            }
            case TileVirtualPixelMethod:
            {
              p=AcquireCacheVirtualNexus(image,TileX(cache_info->columns,x+u),
                TileY(cache_info->rows,y+v),1UL,1UL,virtual_nexus,exception);
              break;
            }
          ...
          *q++=(*p);
          if (indexes != (IndexPacket *) NULL)
            {
              nexus_indexes=GetNexusIndexes(cache_info,virtual_nexus);
      ...
      */
      p=AcquireCacheVirtualNexus(image,x+u,y+v,(unsigned long) length,1UL,
  ...
  DestroyCacheNexus(cache_info,virtual_nexus);
  return(pixels);
}

/*
...
static inline MagickOffsetType ReadCacheRegion(CacheInfo *cache_info,
  ...
  unsigned char *buffer)
{
  ...
  {
#if !defined(MAGICKCORE_HAVE_PREAD)
    count=read(cache_info->file,buffer+i,MagickMin(length-i,(MagickSizeType)
      SSIZE_MAX));
#else
    count=pread(cache_info->file,buffer+i,MagickMin(length-i,(MagickSizeType)
...
static inline MagickOffsetType WriteCacheRegion(CacheInfo *cache_info,
  const MagickOffsetType offset,const MagickSizeType length,
  const unsigned char *buffer)
{
  ...
  {
#if !defined(MAGICKCORE_HAVE_PWRITE)
    count=write(cache_info->file,buffer+i,MagickMin(length-i,(MagickSizeType)
      SSIZE_MAX));
#else
    count=pwrite(cache_info->file,buffer+i,MagickMin(length-i,(MagickSizeType)
      SSIZE_MAX),(off_t) (offset+i));
#endif
    if (count > 0)
      continue;
  ...
  CacheInfo *source_info,ExceptionInfo *exception)
{
       ...
       (source_info->colorspace == CMYKColorspace)))
    {
      ...
      */
      ...
      for (y=0; y < (long) rows; y++)
      {
        ...
        count=ReadCacheRegion(source_info,source_info->offset+source_offset,
          length,(unsigned char *) indexes);
        if ((MagickSizeType) count != length)
          break;
        offset-=cache_info->columns*sizeof(*indexes);
        count=WriteCacheRegion(cache_info,cache_info->offset+offset,length,
  ...
  */
  ...
  for (y=0; y < (long) rows; y++)
  {
    ...
    count=ReadCacheRegion(source_info,source_info->offset+source_offset,length,
       ...
       (source_info->colorspace == CMYKColorspace)))
    {
      ...
      */
      ...
      for (y=0; y < (long) rows; y++)
      {
        ...
        count=ReadCacheRegion(source_info,source_info->offset+offset,length,
          (unsigned char *) indexes);
        if ((MagickSizeType) count != length)
          break;
...
MagickExport MagickBooleanType GetCacheInfo(Cache *cache)
{
  ...
  */
  cache_info->number_views=CacheViewReserve+2*GetCacheViewMaximumThreads();
  ...
  const unsigned long nexus)
{
  ...
  */
  ...
  if (IsNexusInCore(cache_info,nexus) != MagickFalse)
    return(pixels);
  if (ReadCachePixels(cache_info,nexus,&image->exception) == MagickFalse)
    return((PixelPacket *) NULL);
  if ((cache_info->storage_class == PseudoClass) ||
      (cache_info->colorspace == CMYKColorspace))
...
MagickExport unsigned long GetNexus(Cache cache)
{
  ...
  for (id=1; id < (long) cache_info->number_views; id++)
    if (cache_info->nexus_info[id].reserve == MagickFalse)
      {
        cache_info->nexus_info[id].reserve=MagickTrue;
        RelinquishSemaphoreInfo(cache_info->semaphore);
...
%                                                                             %
%                                                                             %
+   G e t P i x e l C a c h e E x t e n t                                     %
%                                                                             %
%                                                                             %
%                                                                             %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  GetPixelCacheExtent() returns the extent of the pixels associated with the
%  last call to SetImagePixels() or GetImagePixels().
%
%  The format of the GetPixelCacheExtent() method is:
%
%      MagickSizeType GetPixelCacheExtent(const Image *image)
%
%  A description of each parameter follows:
%
%    o image: the image.
%
*/
MagickExport MagickSizeType GetPixelCacheExtent(const Image *image)
{
  CacheInfo
    *cache_info;
  ...
  extent=(MagickSizeType) nexus_info->columns*nexus_info->rows;
  if (extent == 0)
    return((MagickSizeType) cache_info->columns*cache_info->rows);
  return(extent);
}
...
%
%      MagickBooleanType ModifyCache(Image *image)
%
%  A description of each parameter follows:
%
%    o image: the image.
...
static MagickBooleanType ModifyCache(Image *image)
{
  ...
  cache_info=(CacheInfo *) image->cache;
  status=CloneCacheNexus(cache_info,clone_info);
...
static MagickBooleanType ExtendCache(Image *image,MagickSizeType length)
{
  ...
  count=WriteCacheRegion(cache_info,offset,1,(const unsigned char *) "");
  ...
  switch (cache_info->type)
  {
    ...
    case DiskCache:
    {
      ...
      */
      ...
      for (y=0; y < (long) rows; y++)
      {
        count=ReadCacheRegion(cache_info,cache_info->offset+number_pixels*
          sizeof(PixelPacket)+offset*sizeof(*indexes),length,(unsigned char *)
          indexes);
        if ((MagickSizeType) count < length)
          break;
        indexes+=nexus_info->columns;
  ...
  switch (cache_info->type)
  {
    ...
    case DiskCache:
    {
      ...
      */
      ...
      for (y=0; y < (long) rows; y++)
      {
        count=ReadCacheRegion(cache_info,cache_info->offset+offset*
          sizeof(*pixels),length,(unsigned char *) pixels);
        if ((MagickSizeType) count < length)
          break;
        pixels+=nexus_info->columns;
        offset+=cache_info->columns;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  SetCacheNexus() allocates an region to store image pixels as defined by the
%  region rectangle and returns a pointer to the region.  This region is
  ...
  const unsigned long nexus)
{
  ...
  */
  if (SyncImagePixelCache(image) == MagickFalse)
    return((PixelPacket *) NULL);
  cache_info=(CacheInfo *) image->cache;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  SyncImagePixelCache() ensures that the pixel cache morphology matches that
%  of the image, in particular, the image storage class, colorspace, width,
%  and height.
%
...
static inline MagickBooleanType IsCacheSynchronized(const Image *image)
{
  CacheInfo
    *cache_info;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  SetPixelCache() allocates an region to store image pixels as defined by the
%  region rectangle and returns a pointer to the region.  This region is
%  subsequently transferred from the pixel cache with SyncPixelCache().  A
%  pointer to the pixels is returned if the pixels are transferred, otherwise
%  a NULL is returned.
...
static MagickBooleanType SyncCache(Image *image)
{
  ...
  cache_info=(CacheInfo *) image->cache;
  assert(cache_info->signature == MagickSignature);
  if (IsCacheSynchronized(image) != MagickFalse)
    return(MagickTrue);
  return(OpenCache(image,IOMode,&image->exception));
}

/*
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  switch (cache_info->type)
  {
    ...
    case DiskCache:
    {
      ...
      */
      ...
      for (y=0; y < (long) rows; y++)
      {
        count=WriteCacheRegion(cache_info,cache_info->offset+number_pixels*
          sizeof(PixelPacket)+offset*sizeof(*indexes),length,(unsigned char *)
          indexes);
        if ((MagickSizeType) count < length)
          break;
  ...
  switch (cache_info->type)
  {
    ...
    case DiskCache:
    {
      ...
      */
      ...
      for (y=0; y < (long) rows; y++)
      {
        count=WriteCacheRegion(cache_info,cache_info->offset+offset*
          sizeof(*pixels),length,(unsigned char *) pixels);
        if ((MagickSizeType) count < length)
          break;
        pixels+=nexus_info->columns;
