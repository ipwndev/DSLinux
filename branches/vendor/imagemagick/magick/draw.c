  ...
  const PrimitiveInfo *primitive_info)
{
  ...
  polygon_info=(PolygonInfo **) AcquireQuantumMemory(
    GetCacheViewMaximumThreads(),sizeof(*polygon_info));
  if (polygon_info == (PolygonInfo **) NULL)
    return((PolygonInfo **) NULL);
  (void) ResetMagickMemory(polygon_info,0,GetCacheViewMaximumThreads()*
    sizeof(*polygon_info));
  ...
  if (primitive_info->coordinates == 1)
    ...
    if (status == MagickFalse)
      continue;
        ...
        q=GetCacheViewPixels(image_view[id],x,y,(unsigned long) (stop-x+1),1);
        if (q == (PixelPacket *) NULL)
          {
            status=MagickFalse;
            continue;
  ...
  */
  ...
  for (y=(long) (bounds.y1+0.5); y <= (long) (bounds.y2+0.5); y++)
  {
    ...
    if (status == MagickFalse)
      continue;
    id=GetCacheViewThreadId();
    q=GetCacheViewPixels(image_view[id],start,y,(unsigned long) (stop-start+1),
      1);
    if (q == (PixelPacket *) NULL)
      {
        status=MagickFalse;
  ...
  switch (primitive_info->primitive)
  {
    ...
    case ImagePrimitive:
    {
      ...
      if (composite_image->matte == MagickFalse)
        (void) SetImageAlphaChannel(composite_image,ResetAlphaChannel);
