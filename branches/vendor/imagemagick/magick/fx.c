  ...
  const long x,const long y,const char *expression,ExceptionInfo *exception)
{
  ...
  switch (*symbol)
  {
      ...
      if (LocaleCompare(symbol,"depth") == 0)
        {
          ...
          (void) FormatMagickString(key,MaxTextExtent,"%s.%p.%ld",symbol,image,
            (long) channel);
      ...
      if ((LocaleCompare(symbol,"image.depth") == 0) ||
          ...
          (LocaleCompare(symbol,"image.standard_deviation") == 0))
        {
          ...
          */
          ...
          if ((LocaleCompare(symbol,"image.minima") == 0) ||
              (LocaleCompare(symbol,"image.maxima") == 0))
            {
              double
                maxima,
              ...
              (void) GetImageChannelRange(image,channel,&minima,&maxima,
                exception);
              if (LocaleCompare(symbol,"image.minima") == 0)
                (void) FormatMagickString(statistic,MaxTextExtent,"%g",minima);
              if (LocaleCompare(symbol,"image.maxima") == 0)
                (void) FormatMagickString(statistic,MaxTextExtent,"%g",maxima);
            }
          if ((LocaleCompare(symbol,"image.mean") == 0) ||
              (LocaleCompare(symbol,"image.standard_deviation") == 0))
            {
              double
                mean,
              ...
              (void) GetImageChannelMean(image,channel,&mean,
                &standard_deviation,exception);
              if (LocaleCompare(symbol,"image.mean") == 0)
                (void) FormatMagickString(statistic,MaxTextExtent,"%g",mean);
              if (LocaleCompare(symbol,"image.standard_deviation") == 0)
    ...
    case 'm':
    {
      if (LocaleCompare(symbol,"maxima") == 0)
        {
          double
            maxima,
            minima;
      ...
      if (LocaleCompare(symbol,"minima") == 0)
        {
          double
            maxima,
            minima;
  ...
  *beta=0.0;
  ...
  switch (*expression)
  {
    ...
    case 'l':
    {
      ...
      if (LocaleNCompare(expression,"log2",4) == 0)
        {
          alpha=FxEvaluateSubexpression(fx_info,channel,x,y,expression+4,beta,
            exception);
  ...
  ExceptionInfo *exception)
{
  ...
  fx_info=(FxInfo **) AcquireQuantumMemory(GetCacheViewMaximumThreads(),
    sizeof(*fx_info));
  if (fx_info == (FxInfo **) NULL)
    return((FxInfo **) NULL);
  (void) ResetMagickMemory(fx_info,0,GetCacheViewMaximumThreads()*
    sizeof(*fx_info));
  if (*expression != '@')
  ...
  const double angle,ExceptionInfo *exception)
{
  ...
  */
  ...
  (void) SetImageAlphaChannel(picture_image,ResetAlphaChannel);
...
#define SepiaToneImageTag  "SepiaTone/Image"
  ...
  */
  status=MagickTrue;
  image_view=AcquireCacheViewThreadSet(image);
  sepia_view=AcquireCacheViewThreadSet(sepia_image);
  for (y=0; y < (long) image->rows; y++)
  {
    register const PixelPacket
      *p;
    ...
    id=GetCacheViewThreadId();
    p=AcquireCacheViewPixels(image_view[id],0,y,image->columns,1,exception);
    q=SetCacheViewPixels(sepia_view[id],0,y,sepia_image->columns,1);
    if ((p == (const PixelPacket *) NULL) || (q == (PixelPacket *) NULL))
      {
        status=MagickFalse;
        continue;
    ...
    if (SyncCacheView(sepia_view[id]) == MagickFalse)
      status=MagickFalse;
    if (SetImageProgress(image,SepiaToneImageTag,y,image->rows) == MagickFalse)
      status=MagickFalse;
  }
  sepia_view=DestroyCacheViewThreadSet(sepia_view);
  image_view=DestroyCacheViewThreadSet(image_view);
