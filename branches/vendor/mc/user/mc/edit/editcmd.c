...
edit_save_file (WEdit *edit, const char *filename)
{
    ...
    if ((fd = mc_open ((char *) filename, O_WRONLY)) == -1) {
	/*
	...
	free (savedir);
	fd = mc_mkstemp (&savename, saveprefix, NULL);
	free (saveprefix);
	if (!savename)
	    return 0;
	/* FIXME:
	 * Close for now because mc_mkstemp use pure open system call 
    ...
    if ((fd =
	 mc_open ((char *) savename, O_CREAT | O_WRONLY | O_TRUNC,
...
/* pipe save */
	...
	if (mc_rename ((char *) filename, catstrs (filename, option_backup_ext, 0))
	    == -1)
	    goto error_save;
    if (this_save_mode != EDIT_QUICK_SAVE)
	if (mc_rename (savename, (char *) filename) == -1)
	    goto error_save;
    free (savename);
    return 1;
  error_save:
...
#define DLG_Y 10
	...
	{quick_input, 23, DLG_X, 5, DLG_Y, 0, 9,
	 0, 0, &str_result, XV_WLAY_DONTCARE, "edit-backup-ext"},
...
edit_save_as_cmd (WEdit *edit)
{
	...
	if (!*exp) {
		...
		if ((file = mc_open ((char *) exp, O_RDONLY)) != -1) {
		    /* the file exists */
		    mc_close (file);
...
//	 "[Input Line Keys]", 0 /*quick_widgets */ };
	...
	/* Sometimes menu can hide replaced text. We don't like it */
...
edit_replace_dialog (WEdit * edit, char **search_text, char **replace_text, char **arg_order)
{
    ...
    int tsearch_set_bookmarks = search_create_bookmark;
...
//	 0, 0, NULL},
	...
	{quick_checkbox, 33, REPLACE_DLG_WIDTH, 8, REPLACE_DLG_HEIGHT, N_("set &Bookmarks"), 0, 0,
	 0, 0, XV_WLAY_DONTCARE, NULL},
//	 0, 0, NULL},
...
//	 0, 0, 0},
	{quick_input, 3, REPLACE_DLG_WIDTH, 5, REPLACE_DLG_HEIGHT, "", 52, 0, 0,
	 0, XV_WLAY_BELOWCLOSE, "search-string"},
//	 0, "edit-replace"},
...
//	 0, 0},
    ...
    quick_widgets[5].result = &tsearch_set_bookmarks;

    quick_widgets[6].result = &treplace_backwards;
...
//	    *search_text = *(quick_widgets[INPUT_INDEX + 4].str_result);
	    ...
	    search_create_bookmark = tsearch_set_bookmarks;
	    return;
	} else {
	    *arg_order = NULL;
	    *replace_text = NULL;
...
// 	 0, 0, NULL },
	{quick_checkbox, 33, SEARCH_DLG_WIDTH, 5, SEARCH_DLG_HEIGHT, N_("&Backwards"), 0, 0,
	 0, 0, XV_WLAY_DONTCARE, NULL },
//	 0, 0, NULL},
	{quick_checkbox, 33, REPLACE_DLG_WIDTH, 6, REPLACE_DLG_HEIGHT, N_("set &Bookmarks"), 0, 0,
	 0, 0, XV_WLAY_DONTCARE, NULL},
//	 0, 0, NULL},
...
//	 0, 0},
    ...
    quick_widgets[4].result = &tsearch_set_bookmarks;

    quick_widgets[5].result = &treplace_regexp;

    quick_widgets[6].result = &treplace_whole;
    quick_widgets[7].result = &treplace_case;
    quick_widgets[8].str_result = search_text;
    quick_widgets[8].text = *search_text;
...
void edit_search_cmd (WEdit * edit, int again)
{
	...
	if (*exp) {
		...
		for (;;) {
		    ...
		    if (l != l_last) {
		    if (!book_mark_query_color (edit, l, BOOK_MARK_COLOR) || !book_mark_query_color (edit, l, BOOK_MARK_FOUND_COLOR))
			{
...
/* Return a null terminated length of text. Result must be free'd */
unsigned char *
 ...
 */
static int
e_close_error_pipe (int error, char *text)
{
    char *title;
			     ...
			     int sout, int ifnhi, int pad)
{
	...
	if (!(script_home = fopen (s, "r"))) {
	    s = catstrs (home_dir, EDIT_DIR "/edit.", scr, ".rc", 0); /* user script short name */
	    if (!(script_home = fopen (s, "r"))) {
...
/* 0*/	{quick_button, 6, 10, 15, EXT_SCRIPT_H, N_("&Cancel"), 0, B_CANCEL, 0, 
	0, XV_WLAY_RIGHTOF, NULL},
/* 1*/	{quick_button, 2, 10, 15, EXT_SCRIPT_H, N_("&OK"), 0, B_ENTER, 0,
	 0, XV_WLAY_RIGHTOF, NULL},
/* 2*/	{quick_radio, 4, EXT_SCRIPT_W, 11, EXT_SCRIPT_H, "", 3, sout, &sout,
         sout_str, XV_WLAY_RIGHTOF, "extscript-dlg-stdout"},
/* 3*/	{quick_label, 3, EXT_SCRIPT_W, 10, EXT_SCRIPT_H, N_("Output:"), 0, 0, 0,
         0, XV_WLAY_RIGHTOF, NULL},
/* 4*/	{quick_checkbox, 3, EXT_SCRIPT_W, 8, EXT_SCRIPT_H, N_("Pad if column highliting"), 0, 0, 0,
	 0, XV_WLAY_RIGHTOF, NULL},
/* 5*/	{quick_checkbox, 3, EXT_SCRIPT_W, 7, EXT_SCRIPT_H, N_("Whole file if no highlited block"), 0, 0, 0,
	 0, XV_WLAY_RIGHTOF, NULL},
/* 6*/	{quick_input, 14, EXT_SCRIPT_W, 5, EXT_SCRIPT_H, "", EXT_SCRIPT_W - 17, 0, 0,
	 0, XV_WLAY_RIGHTOF, "extscript-dlg-pipe"},
/* 7*/	{quick_input, 14, EXT_SCRIPT_W, 4, EXT_SCRIPT_H, "", EXT_SCRIPT_W - 17, 0, 0,
	 0, XV_WLAY_RIGHTOF, "extscript-dlg-script"},
/* 8*/	{quick_radio, 4, EXT_SCRIPT_W, 4, EXT_SCRIPT_H, "", 2, type, &type,
         type_str, XV_WLAY_RIGHTOF, "extscript-dlg-type"},
/* 9*/	{quick_label, 3, EXT_SCRIPT_W, 3, EXT_SCRIPT_H, N_("Script:"), 0, 0, 0,
         0, XV_WLAY_RIGHTOF, NULL},
/*10*/	{quick_label, 14, EXT_SCRIPT_W, 3, EXT_SCRIPT_H, N_("Name/command line"), 0, 0, 0,
	 0, XV_WLAY_RIGHTOF, 0},
	{0}};
