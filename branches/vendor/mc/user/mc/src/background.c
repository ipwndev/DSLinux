...
#ifdef USE_NETCODE
#	ifdef __BEOS__
#		include </boot/develop/headers/be/bone/sys/socket.h>
//#		include <sys/socket.h> /* Dan0 */ //--Olegrch
//#		include <net/socket.h> /* PE5 */ //--Olegrch
#	else
...
#ifdef __BEOS__
    if (socketpair (AF_INET, SOCK_STREAM, 0, comm) == -1)
#else /*__BEOS__*/
