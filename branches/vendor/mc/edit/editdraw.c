...
static void status_string (WEdit * edit, char *s, int w, int fill)
{
    ...
    if (clock_in_editor && menubar_visible && get_clock_type())
	/* When we shows clock - status line must be shorter */ //--Olegarch
	sprintf (s, "[%c%c%c%c] %2ld:%3ld/%3ld=%3ld+%2ld -*%-4ld/%4ldb=%s",
	...
	/* The field lengths just prevent the status line from shortening too much */
        sprintf (s, "[%c%c%c%c] C:%3ld L:[%3ld/%3ld %3ld+%2ld] *(%-4ld/%4ldb)= %s",
...
edit_status (WEdit *edit)
{
	...
	t = w - 20;
	if (t > 1)		/* sprintf() must write at least '\000' */
		     ...
		     long end_col)
{
    ...
    if (book_mark_query_all (edit, edit->start_line + row, &book_mark) <= 0)
	book_mark = -1;
