...
extern int dir_symlinks_as_files;
...
#ifdef HAVE_DLGSWITCH
    int dlgnum = dlglist_get_dlg_num_if_it_is_active(
	DLG_VIEWER, filename );
    if (dlgnum >= 0) {
	dlglist_dialog_switch( DIR_NTH, dlgnum );
	return 0;
    }
#endif
...
view_file (char *filename, int plain_view, int internal)
{
    ...
    if (view_save_line) {
	return view_file_at_line (filename, plain_view, internal, view_get_saved_line((char *) vfs_canon (filename)));
    } else
...
static void do_view_cmd (WPanel *panel, int normal)
{
    ...
    /* 
     * If F3/F13 key press on directory - calculate size of them, 
     * fix by Hatred aka Hatred <hatred@iatp.festu.ru>
     */

    if (S_ISDIR (selection (panel)->buf.st_mode) || link_isdir (selection (panel))) {
                if (panel->dirs_marked) 
// if dirs selected, we use dirsizes_cmd	--Hatred suggestion
		    dirsizes_cmd();
		else {
// else - we must calculate only one dir	--Olegarch
		    if (strcmp (selection (panel)->fname, "..") != 0) {
			off_t marked;
			unsigned long long total = 0ULL;
//			double total = 0.0l;
			compute_dir_size (selection (panel)->fname, &marked, &total);
			selection (panel)->buf.st_size = (off_t)total;
			selection (panel)->dir_size = (unsigned long long)total; // WARNING!
			selection (panel)->f.dir_size_computed = 1;
		    }
		}
...
if (cpanel->marked <= 1)
    {
    save_cwds_stat ();
    if (panel_operate (cpanel, OP_RENAME, NULL)) {
	update_panels (UP_OPTIMIZE, UP_KEEPSEL);
	repaint_screen ();
    }
}
...
if (panel->panel_charset)
    convert_from_input_panel (panel, dir);
#endif /* ! HAVE_CHARSETS */
...
void reverse_selection_cmd_panel (WPanel *panel)
{
	...
	if (S_ISDIR (file->buf.st_mode) ||
	    (!dir_symlinks_as_files && link_isdir ( &(cpanel->dir.list [i]) )))
	    continue;
...
void unselect_cmd_panel (WPanel *panel)
{
    ...
    if ((*reg_exp_t) && reg_exp_t [strlen(reg_exp_t) - 1] == PATH_SEP) {
        dirflag = 1; fileflag = 0; // csak dir --Arpi
        reg_exp_t [strlen(reg_exp_t) - 1] = 0;
    }
    if(*reg_exp_t == 0) reg_exp_t="*";
...
/* REMAKE calculatind dir sizes. Hatred */
...
//	    total = 0.0l;
	    ...
	    panel->dir.list [i].dir_size = (unsigned long long)total; // WARNING!
