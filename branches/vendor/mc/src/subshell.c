...
/* Don't allow ctrl+o (panels ON) if shell already running a command */
...
const char subshell_switch_key = XCTRL('o') & 255;
...
//	    sprintf (tcsh_fifo, "/tmp/mc.pipe.%d", getpid ());
           ...
           tcsh_fifo = copy_strings (tmpdir, "/mc.pipe.", getpid ());
           printf("--> %s <--\r\n", tcsh_fifo);
...
//	char precmd2[80];
	...
	sprintf (precmd, " PROMPT_COMMAND='pwd>&%d;kill -STOP $$'\n",
		 subshell_pipe[WRITE]);
//	sprintf (precmd2," alias mc='echo Do not run mc from mc!'\n");
	goto write_it;
...
//	write (subshell_pty, precmd2, strlen (precmd2));
    }
...
int exit_subshell (void)
{
	...
	sprintf (pty_buffer, "%s/mc.pipe.%d", tmpdir, getpid ());
...
static int feed_subshell (int how, int fail_on_error)
{
	...
	if (FD_ISSET (subshell_pty, &read_set))
	    ...
	    // block ctrl+o //--Arpi
	        ...
	        if (bytes>=1 && pty_buffer[0] == subshell_switch_key &&
                    subshell_ready==TRUE) {
		        subshell_state = INACTIVE;
		        return TRUE;
                    }
	    // remove ctrl+o codes from buffer //--Arpi
		for (i=0,j=0; i<bytes; ++i)
		    if (pty_buffer[i] != subshell_switch_key)
                        pty_buffer[j++]=pty_buffer[i];

		if(j>0) {
                    write (subshell_pty, pty_buffer, j);
                }
	        subshell_ready = FALSE;
              } else {
		for (i=0; i<bytes; ++i)
...
static int pty_open_master (char *pty_name)
{
	...
	|| !(slave_name = (char *) ptsname (pty_master)))  /* Get slave's name */
    {
	close (pty_master);
	return -1;
    }
