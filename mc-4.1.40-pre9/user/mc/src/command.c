...
int wildcard_cmp_names(char *n,char *m){
next:
  if(m[0]=='*'){
    m++;
    if(m[0]==0) return 1;
...
static int examine_cd (char *path)
{
    ...
    *r = 0;
    ...
    if(!result && strchr(q,92)){
      // quoted dirname
      char *q2=xmalloc(strlen(q)+1, "examine_cd_q2");
      char *s=q;
      char *d=q2;
      int c;
      do{
...
static int 
command_callback (Dlg_head *h, WCommand *cmd, int msg, int par)
{
    switch (msg){
    case WIDGET_FOCUS:
...
//    in = input_new (y, x, DEFAULR_COLOR, cols, "", "cmdline");
    in = input_new (y, x, NULL, cols, "", "cmdline"); /*Command line color here*/ //--Olegarch
    cmd->input = *in;
    free (in);

    /* Add our hooks */
    cmd->old_callback = (callback_fn) cmd->input.widget.callback;
    cmd->input.widget.callback = (int (*) (Dlg_head *, void *, int, int)) command_callback;
