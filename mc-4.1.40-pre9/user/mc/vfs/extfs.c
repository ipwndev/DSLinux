...
void extfs_fill_names (void (*func)(char *))
{
    ...
    while (a){
       name = copy_strings (a->name ? a->name : "",
                           "#", extfs_prefixes [a->fstype],
                           PATH_SEP_STR, a->current_dir->name, NULL);
//	name = copy_strings ((a->name ? a->name : ""),"#" ,extfs_prefixes [a->fstype], 
//			     PATH_SEP_STR, a->current_dir->name, 0);
...
static FILE *open_extfs_archive (int fstype, char *name, struct extfs_archive **pparc)
{
	...
	if (mc_stat (name, &mystat) == -1) return NULL;
    ...
    cmd = copy_strings (LIBDIR "extfs", PATH_SEP_STR, extfs_prefixes [fstype], 
    ...
    if (mode & 0004)
    	mode |= 0001;
    mode |= S_IFDIR;
    root_entry = generate_entry (current_archive, PATH_SEP_STR, NULL, mode);
...
//    pclose (extfsd);
   ...
   /* extfs 'list' didn't return 0 */
   if (pclose(extfsd)!=0) {
       /* FIXME: Should clean everything one day */
       free (buffer);
       close_error_pipe (1,
                        _("Error reading archive"));
       return -1;
    }
...
char *extfs_analysis (char *name, char **archive, int *fstype, int is_dir)
{
	...
	*archive = 0;
	...
	char c;
	p = strchr (name + strlen (extfs_prefixes [*fstype]) + 1, PATH_SEP);
	if (p){
	    c = *p;
	    *p = 0;
	}
...
{
	//FixMe: p+1 can be less then extfs_prefixes[*fstype]. --Hatred, Fixed.
	//FixMe: bugs forever :-) find it and fix!             --Hatred
	if(*p=='#' && strlen(p+1)>=strlen(extfs_prefixes[*fstype]) && !strncmp(p+1,extfs_prefixes[*fstype],strlen(extfs_prefixes[*fstype])))
	{
		if(p+1+strlen(extfs_prefixes[*fstype]))
		{
			local = strdup(p+1+strlen(extfs_prefixes[*fstype]));	
		}
		else // Hmmm... Does this part of code work at sometimes or other? --Hatred
		{
			local = strdup(PATH_SEP_STR);	
		}
		     ...
		     struct extfs_loop_protect *list, int make_dirs, int make_file)
{
    ...
    if (*name == PATH_SEP) { /* Handle absolute paths */
    	name++;
	...
	*q = 0;
	...
	if (strcmp (p, ".")){
		...
		if (c == PATH_SEP && !S_ISDIR (pent->inode->mode)){
		    *q = c;
		    notadir = 1;
		    return NULL;
		}
...
static int
extfs_cmd (const char *extfs_cmd, struct extfs_archive *archive,
	   struct extfs_entry *entry, const char *localname)
{
    char *file;
    char *quoted_file;
...
void extfs_run (char *file)
{
	...
	return;
    q = name_quote (((*p == PATH_SEP) ? p + 1 : p), 0);
    
    archive_name = name_quote (get_archive_name(archive), 0);
    cmd = copy_strings (LIBDIR "extfs", PATH_SEP_STR, extfs_prefixes [archive->fstype], 
...
static void *extfs_open (char *file, int flags, int mode)
{
	...
	suffix = copy_strings ("-", entry->name, NULL);

	if ((local_handle =
	     mc_mkstemp (&local_filename, "extfs", suffix)) == -1) {
	    /* fallback for the case if the filename is too long */
	    local_handle = mc_mkstemp (&local_filename, "extfs", NULL);
	}
	free (suffix);
...
#if 0
#define ERRNOR(a, b) do { extfserrno = a; return b; } while (0)
#else
#define ERRNOR(a, b) { extfserrno = a; return b; }
#endif
...
static int extfs_unlink (char *path)
{
    ...
    if ((q = extfs_get_path (path, &archive, 0, 0)) == NULL)
	return -1;
    entry = extfs_find_entry (archive->root_entry, q, 0, 0);
    if (entry == NULL)
    	return -1;
...
static int extfs_chdir (char *path)
{
    ...
    res = copy_strings (entry->inode->archive->name, 
	                    "#", extfs_prefixes [entry->inode->archive->fstype],
        			    p, NULL);
    free (p);
    extfserrno = 0;
...
for(p=path+strlen(path);p>=path;p--)
{
    if(*p=='#')
    	for (i = 0; i < extfs_no; i++)
        {
			...
			if((p+1)
           		&& strlen(p+1)>=j
           		&& !strncmp(p+1,extfs_prefixes [i], j)) return i;
    	}
}
