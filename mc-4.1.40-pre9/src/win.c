...
/* "$Id: win.c,v 1.2 1998/03/25 05:17:01 unammx Exp $" */
...
//void sprint_bytesize (char *buffer, int size, int scale) // WARNING!
void sprint_bytesize (char *buffer, unsigned long long size, int scale)
{
    char scales[] = " kMGT";
...
//	    size = (size + 512) / 1024;
	while (size > 9999ULL && scale < sizeof (scales)) {
	    size = (size + 512ULL) / 1024ULL;
	    scale ++;
	}
    }
    if (scale > 0)
//	sprintf (buffer, "%4d %cb", size, scales[scale]);
	sprintf (buffer, "%4lu %cb", size, scales[scale]);
    else
	sprintf (buffer, "%4lu b ", size);
//	sprintf (buffer, "%4d b ", size);
}

void print_bytesize (unsigned long long size, int scale)
			 ...
			 movefn bottomfn)
{
	...
	case 'd':
	    ...
	    return 1;
    case KEY_PPAGE | KEY_M_CTRL:
	case 'g':
	    (*topfn)(data, 0);
	    return 1;
    case KEY_NPAGE | KEY_M_CTRL:
	case 'G':
	    (*bottomfn)(data, 0);
	    return 1;
	}
