...
/* "$Id: layout.c,v 1.8 1998/05/20 02:20:09 unammx Exp $" */
...
/* Set if you want the clock shown in editor's status line */
int clock_in_editor = 0;

/* The starting line for the output of the subprogram */
int output_start_y = 0;

/* Set to show size status of current mountpoint */
int show_mountpoint_info = 1;


extern int panel_scrollbar;
...
/* Internal variables */
static int _double_frames;
static int _panel_scrollbar;

static int _show_mountpoint_info;
...
static int _clock_in_editor;
static int _permission_mode;
static int _filetype_mode;
static int _lynx_movement;
...
static struct {
    ...
    { N_("&Show mountpoint info"), &show_mountpoint_info,   0, "sm" },
    { N_("l&Ynx-like motion"), &navigate_with_arrows,   0, "lm" },
    { N_("Panel &Scrollbar"),  &panel_scrollbar, 0, "ps" },
    { N_("&Double frames"),    &double_frames,   0, "df" },
    { N_("&Clock in editor"),  &clock_in_editor, 0, "ec" },
...
static int b2left_cback (int action, void *data)
{
	...
	_equal_split = 0;
	check_options [11].widget->state = check_options [11].widget->state & ~C_BOOL;
	dlg_select_widget (layout_dlg, check_options [11].widget);
...
static int layout_callback (struct Dlg_head *h, int Id, int Msg)
{
    switch (Msg) {
    case DLG_DRAW:
/*When repainting the whole dialog (e.g. with C-l) we have to update everything*/
	...
	draw_box (h, 2, 5 + first_width, 13, second_width);
    ...
    case DLG_POST_KEY:
	_filetype_mode = check_options [13].widget->state & C_BOOL;
	_permission_mode = check_options [12].widget->state & C_BOOL;
	_equal_split = check_options [11].widget->state & C_BOOL;
	_menubar_visible = check_options [10].widget->state & C_BOOL;

	_command_prompt = check_options [8].widget->state & C_BOOL;
	_keybar_visible = check_options [7].widget->state & C_BOOL;
	_message_visible = check_options [6].widget->state & C_BOOL;
//	_xterm_hintbar = check_options [5].widget->state & C_BOOL;
	_xterm_title = check_options [5].widget->state & C_BOOL;
	_clock_in_editor = check_options [4].widget->state & C_BOOL;

	_double_frames = check_options [3].widget->state & C_BOOL;
	_panel_scrollbar = check_options [2].widget->state & C_BOOL;
	_lynx_movement = check_options [1].widget->state & C_BOOL;
	_show_mountpoint_info = check_options [0].widget->state & C_BOOL;
...
static void init_layout (void)
{
    ...
    if (!i18n_layt_flag)
    {
		 ...
		 */
		l1 = 14 + strlen (ok_button)   /* 14 - all brackets and inner space */
    ...
    layout_dlg = create_dlg (0, 0, 18, first_width + second_width + 9, 
...
#define XTRACT(i) *check_options[i].variable, check_options[i].text, check_options[i].tkname
    ...
    for (i = 0; i < 11; i++) {
	check_options [i].widget = check_new (13 - i, 7 + first_width, XTRACT(i));
	add_widgetl (layout_dlg, check_options [i].widget, XV_WLAY_BELOWCLOSE);
    }

    if (console_flag) {
	add_widget (layout_dlg,
		    button_new (13, 11,  B_MINUS, NARROW_BUTTON, "&-",
			bminus_cback, 0, NULL));
	add_widget (layout_dlg,
		    button_new (13, 6, B_PLUS, NARROW_BUTTON, "&+", 
			bplus_cback, 0, NULL));
    }

    check_options [12].widget = check_new (10, 6, XTRACT(12));
    add_widgetl (layout_dlg, check_options [12].widget, XV_WLAY_BELOWCLOSE);

    check_options [13].widget = check_new (9, 6, XTRACT(13));
    add_widgetl (layout_dlg, check_options [13].widget, XV_WLAY_BELOWCLOSE);

    _show_mountpoint_info = show_mountpoint_info;
    _double_frames = double_frames;
    _panel_scrollbar = panel_scrollbar;
    _lynx_movement = navigate_with_arrows;
...
//    _xterm_hintbar = xterm_hintbar;
    ...
    _clock_in_editor = clock_in_editor;
    ...
    check_options [11].widget = check_new (5, 6, XTRACT(11));
    old_first_panel_size = -1;
    old_horizontal_split = -1;
    old_output_lines     = -1;
    ...
    add_widget (layout_dlg, check_options [11].widget);
...
#ifdef HAVE_CHARSETS
    if (panel1->panel_charset != panel2->panel_charset) {

	int ii=0;

    	    panelswap (panel_charset);

	for (ii=0;ii<256;ii++){
    	    panelswap (conv_displ[ii]);
    	    panelswap (conv_input[ii]);
    	    panelswap (printable[ii]);
	}
    }
#endif /* HAVE_CHARSETS */
