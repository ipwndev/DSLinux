...
/* "$Id: menu.c,v 1.12 1998/05/20 22:34:16 norbert Exp $" */
...
static void menubar_draw (WMenu *menubar)
{
	...
	if (menubar_visible && get_clock_type()) {
    	    clock_resume();
	    show_clock();
	}
    ...
    for (i = 0; i < items; i++){
	int cnt = 0;
    	unsigned char *text = menubar->menu [i]->name;
// Re: "Colorize menubar hotkeys at least!". Answer: "Done!" ;) //--Olegarch
	if (menubar->active){
	    color = (i == menubar->selected) ? MENU_SELECTED_COLOR:MENU_ENTRY_COLOR;
	    attrset(color);
	}

	widget_move (&menubar->widget, 0, menubar->menu [i]->start_x);

	for (text = menubar->menu [i]->name ; *text; text++)
	{
		if (!cnt)
		{
			if (*text == ' ') cnt = 0; else cnt = 1; // if first char - space, we must colorize next char //--Olegarch
			color = (i == menubar->selected) ? (menubar->active ? MENU_HOTSEL_COLOR:MENU_HOT_COLOR):MENU_HOT_COLOR;
		        attrset(color);
			addch(*text);
		        color = (i == menubar->selected) ? (menubar->active ? MENU_SELECTED_COLOR:MENU_ENTRY_COLOR):MENU_ENTRY_COLOR;
			attrset(color);
...
static void menubar_setsubsel (WMenu *menubar, int off, int step)
{
    const Menu menu = menubar->menu [menubar->selected];
...
static int menubar_handle_key (WMenu *menubar, int key)
{
	...
	if (key == KEY_END || key == XCTRL ('e'))
	    menubar_setsubsel (menubar, 0, -1);
	if (key == KEY_HOME || key == XCTRL ('a'))
	    menubar_setsubsel (menubar, menubar->menu[menubar->selected]->count-1, 1);
...
#ifdef __BEOS__
/* 
   BeOS beterm so stupid :(

   Can you see any alternatives to refresh clock, 
   without redrawing another part of screen? --Olegarch
*/
    mc_refresh();
#endif /*__BEOS__*/
      addstr(timestr);
