...
/* "$Id: dialog.c,v 1.2 1998/04/28 04:29:29 unammx Exp $" */
...
void push_refresh (void (*new_refresh)(void *), void *parameter, int flags)
{
    ...
    new_ = xmalloc (sizeof (Refresh), "push_refresh");
    new_->next = (struct Refresh *) refresh_list;
    new_->refresh_fn = new_refresh;
    new_->parameter = parameter;
    new_->flags     = flags;
    refresh_list = new_;
#ifdef MC_DEBUG
    dbg_message( "\npush_refresh: %p\n", parameter );
...
{
#ifdef MC_DEBUG
    dbg_message( "do_refresh: %d, %X", we_are_background, refresh_list );
#endif
