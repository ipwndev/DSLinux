...
#include <config.h>
/* "$Id: dlg.c,v 1.18 1998/05/06 20:04:16 unammx Exp $" */
#include <string.h>
#include <stdio.h>
#include <stdlib.h>
#include <ctype.h>
...
static void own_vline(char ch, int y, int x, int len)
{
...
/* draw my double box in window */
static void draw_my_double_box (Dlg_head *h, int y, int x, int ys, int xs)
{
    extern int slow_terminal;
...
/* We define style of drawing double frame. If it's PC-style encoding/mapping, 
 * we use one set of chars, if it's koi8-group we use another one.
 */ //--Olegarch

#ifndef PC_DRW_CHARS
////////////// koi8-*

#define DRW_D_HOR 0x00A0
#define DRW_D_VER 0x00A1

#define DRW_D_ULC 0x00A5
#define DRW_D_URC 0x00AB
#define DRW_D_LLC 0x00A8
#define DRW_D_LRC 0x00AE

#else
////////////// ALL OF PC CODEPAGES: cp8**, cp7**, cp4**

#define DRW_D_HOR 0x00CD
#define DRW_D_VER 0x00BA

#define DRW_D_ULC 0x00C8
#define DRW_D_URC 0x00BC
#define DRW_D_LLC 0x00C9
#define DRW_D_LRC 0x00BB

#endif
    waddc (h, y, x, DRW_D_ULC);
    hline (DRW_D_HOR, xs - 2);
    waddc (h, y + ys - 1, x, DRW_D_URC);
    hline (DRW_D_HOR, xs - 2);

    own_vline (DRW_D_VER, h->y+y+1, h->x+x, ys - 2);
    own_vline (DRW_D_VER, h->y+y+1, h->x+x+xs-1, ys - 2);

    waddc (h, y, x + xs - 1, DRW_D_LLC);
    waddc (h, y + ys - 1, x + xs - 1, DRW_D_LRC);
}
...
#ifndef HAVE_SLANG
    if (double_frames)
	draw_my_double_box (h, h->y+y, h->x+x, ys, xs);
    else
	draw_box (h, h->y+y, h->x+x, ys, xs);
#else /* HAVE_SLANG */
    if (double_frames)
	draw_my_double_box (h, h->y+y, h->x+x, ys, xs);
    else
...
int default_proc (Dlg_head *h, int Msg, int Par)
{
    ...
    printf ("Internal error: unhandled message: msg:%d\r\n", Msg);
    return 1;
}
...
common_dialog_repaint (struct Dlg_head *h)
{
    ...
    if (h != midnight_dlg){ // if current dialog - is midnight main screen, we can see underline console    --Olegarch
	attrset (NORMALC);
	dlg_erase (h);
