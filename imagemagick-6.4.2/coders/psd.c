  ...
  Image *image,const unsigned char *blocks,size_t length)
{
  ...
  for (p=blocks; (p >= blocks) && (p < (blocks+length-16)); )
  {
    if (LocaleNCompare((char *) p,"8BIM",4) != 0)
      break;
    p=PushLongPixel(quantum_state.endian,p,&long_sans);
    p=PushShortPixel(quantum_state.endian,p,&id);
    p=PushShortPixel(quantum_state.endian,p,&short_sans);
    p=PushLongPixel(quantum_state.endian,p,&count);
    switch (id)
    {
      case 0x03ed:
      {
        ...
        /*
          Resolution info.
        */
        p=PushShortPixel(quantum_state.endian,p,&resolution);
  ...
  Image *tmp_image,unsigned char *pixels,QuantumType quantum_type)
{
  ...
  for (y=0; y < (long) tmp_image->rows; y++)
  {
    ...
    break;
    (void) ExportQuantumPixels(tmp_image,&quantum_info,quantum_type,pixels);
