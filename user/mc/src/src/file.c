...
char *operation_names [] = { "Copy", "Move", "Rename", "Delete" };
...
static char *op_names [] = { N_(" Copy "), N_(" Move "), N_(" Rename "), N_(" Delete ") };
...
create_op_win (int op, int with_eta)
{
    ...
    x_set_dialog_title (op_dlg, op_names[op]);

////////// BGR
//#ifdef WITH_BACKGROUND
//    add_widgetl (op_dlg, button_new (BY-minus, WX - 14 + eta_offset, B_USER, 
//		    NORMAL_BUTTON, _("&Background"), 0, 0, "back"), XV_WLAY_CENTERROW);
...
//    add_widgetl (op_dlg, button_new (BY-minus, WX - 19 + eta_offset, FILE_ABORT,
    add_widgetl (op_dlg, button_new (BY-minus, WX - 22 + eta_offset, FILE_ABORT,
...
copy_file_file (char *src_path, char *dst_path, int ask_overwrite)
{
 ...
 retry_src_xstat:
	...
	do_reget = -1;		// Always allow REGET'ing  --Arpi
...
#ifdef USE_NETCODE
    /* FXP support */
    if(vfs_file_is_ftp(src_path) && vfs_file_is_ftp(dst_path)){
        int ret;
//        message_1s (1, MSG_ERROR, _(" Doing server-to-server FXP file transfer"));
        if(do_append)
            ret=ftpfs_fxp_copy(src_path,dst_path,O_WRONLY | O_APPEND);
        else
            ret=ftpfs_fxp_copy(src_path,dst_path,O_WRONLY | O_CREAT | O_TRUNC);
        if(ret){
//            message_1s (1, MSG_ERROR, _(" FXP okey!"));
            resources &= ~4; /* copy successful, don't remove target file */
            return_status=0;
            goto fxp_return;
        } 
//        message_1s (1, MSG_ERROR, _(" FXP error... trying standard method"));
    }
#endif /* USE_NETCODE */
 ...
 retry_dst_fstat:
    ...
    mc_refresh ();
    if (return_status != FILE_CONT) goto ret;

// reget'ing local:
    if(do_reget>0){ 
	mc_lseek (source_desc,do_reget,SEEK_SET);
	n_read_total=do_reget;		
	do_reget=0;
    }
...
//-----------------------------------------------------------------------------
	    ...
	    /* 3. Compute ETA */
	    if (secs > 1 || eta_secs == 0.0){
		dt = (tv_current.tv_sec - tv_transfer_start.tv_sec);
		
		if (bps_read_total){
		    eta_secs = ((dt / (double) bps_read_total) * file_size) - dt;
		    bps = bps_read_total / ((dt < 1) ? 1 : dt);
		} else {
		    eta_secs = 0.0;
		    bps = 0;
		}
 ...
 */
static char *op_names1 [] = { N_("1Copy"), N_("1Move"), N_("1Rename"), N_("1Delete") };
...
#   define source_with_path source
	...
	rx.translate = 0;
        if(operation==OP_RENAME||(operation == OP_COPY && copy_local == 1)){
    	    dest = file_mask_dialog (operation, cmd_buf, source, only_one, &do_bg);
        } else {
...
if (new_source && panel->panel_charset && opanel->panel_charset) {
    convert_to_display_panel (panel, new_source);
    convert_from_input_panel (opanel, new_source);
}
    dest_full = concat_dir_and_file (dest_dir, new_source);

    free (new_source);
}
else 
#endif /* ! HAVE_CHARSETS */
	    dest_full = concat_dir_and_file (dest_dir, source);

    	    dest = file_mask_dialog (operation, cmd_buf, dest_full, only_one, &do_bg);
 ...
 clean_up:
    /* Clean up */
    destroy_op_win ();
    if(vfs_file_is_ftp (panel->cwd) ||vfs_file_is_ftp (opanel->cwd))
        ftpfs_flushdir();
    free_linklist (&linklist);
    free_linklist (&dest_dirs);
#if WITH_FULL_PATHS
 ...
 */
void
compute_dir_size (char *dirname, off_t *ret_marked, unsigned long long *ret_total) // WARNING!
//compute_dir_size (char *dirname, off_t *ret_marked, double *ret_total)
{
	...
	if (res != 0) {
        ...
        rotate_dash (); // show - mc hardly works :)

	if (S_ISDIR (s.st_mode)) {
	    off_t subdir_count = 0;
//	    double subdir_bytes = 0;
	    unsigned long long subdir_bytes = 0ULL; // WARNING!
	    ...
	    *ret_total += subdir_bytes;
	    ...
	    (*ret_marked)++;
//	    *ret_total += s.st_size;
	    *ret_total += (unsigned long long)s.st_size; // WARNING!
	}
	free (fullname);
    }
