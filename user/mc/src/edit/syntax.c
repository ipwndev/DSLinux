...
#include "src/regex.h"		/* use_colors */
...
#define syntax_free(x) do {if(x) {free(x); (x)=0;}} while (0)
...
static struct syntax_rule edit_get_rule (WEdit * edit, long byte_index)
{
	...
	for (i = edit->last_get_rule + 1; i <= byte_index; i++) {
		...
		memset(edit->syntax_marker, 0, sizeof (struct _syntax_marker));
...
static FILE *open_include_file (char *filename)
{
    ...
    error_file_name = copy_strings (home_dir, EDIT_DIR PATH_SEP_STR,
				   filename, NULL);
    ...
    free (error_file_name);
    error_file_name = copy_strings (mc_home, PATH_SEP_STR "syntax" PATH_SEP_STR,
				   filename, NULL);
...
static int edit_read_syntax_rules (WEdit * edit, FILE * f)
{
    ...
    r = edit->rules = malloc (MAX_CONTEXTS * sizeof (struct context_rule *));
    memset (r, 0, MAX_CONTEXTS * sizeof (struct context_rule *));
    memset (edit->rules, 0, MAX_CONTEXTS * sizeof (struct context_rule *));
	...
	if (!args[0]) {
		...
		a++;
		c = r[0] = malloc (sizeof (struct context_rule));
		memset (c, 0, sizeof (struct context_rule));
		memset (r[0], 0, sizeof (struct context_rule));
		c->left = (char *) strdup (" ");
		c->right = (char *) strdup (" ");
		num_contexts = 0;
	    } else {
		c = r[num_contexts] = malloc (sizeof (struct context_rule));
		memset (c, 0, sizeof (struct context_rule));
		memset (r[num_contexts], 0, sizeof (struct context_rule));
	    ...
	    c->keyword = malloc (MAX_WORDS_PER_CONTEXT * sizeof (struct key_word *));
	    memset (c->keyword, 0, MAX_WORDS_PER_CONTEXT * sizeof (struct key_word *));
#if 0
	    c->max_words = MAX_WORDS_PER_CONTEXT;
#endif
	    num_words = 1;
	    c->keyword[0] = malloc (sizeof (struct key_word));
	    memset (c->keyword[0], 0, sizeof (struct key_word));
	    fg = *a;
	    if (*a)
		a++;
	    bg = *a;
		...
		break_a;
	    check_a;
	    k = r[num_contexts - 1]->keyword[num_words] = malloc (sizeof (struct key_word));
	    memset (k, 0, sizeof (struct key_word));
	    memset (r[num_contexts - 1]->keyword[num_words], 0, sizeof (struct key_word));
	    if (!strcmp (*a, "whole")) {
		a++;
	...
	for (i = 0; edit->rules[i]; i++) {
	    ...
	    *p = '\0';
	    ...
	    memset (c->keyword_first_chars, 0, strlen (first_chars) + 2);
