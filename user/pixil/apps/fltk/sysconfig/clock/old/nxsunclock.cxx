...
static int city_size;		// Size of the city list
static city_rec_t query_rec;	// Query record
static h_data_rec *city_list;	// List of "near" cities

// Static functions
static void parsecity(char *data, city_rec_t * crec);
static int GetCityList(char *zndb, int latitude, int longitude);
static void ExpandTimeZone(char *dst, char *src, int dsize);
...
nxSunclock::GetAppPrefs(void)
{
     ...
     *pardb;			// Name of the default database
...
nxSunclock::MakeTimewin(int X, int Y, int w, int h)
{
    ...
    //_timewin->color(NxApp::Instance()->getGlobalColor(APP_BG));
    // Add all of the widgets

    d_y = Y + BUTTON_Y - _winY;
    d_x = X + BUTTON_X;
    {
	// Add the Atomic Clock widget
#if 0
//              if (net_if_up("eth0", 0) || net_if_up("ppp0", 0))
//              {
	NxButton *o;		// Button widget

	nbuttons = 3;

	o = new NxButton(d_x, d_y, BUTTON_WIDTH, BUTTON_HEIGHT, "Atomic");
	o->when(FL_WHEN_RELEASE);
//                      o->callback(get_atomic_time_cb);
	d_x += 63;
    }				// end of if
    else
    {
#endif
	// Set the button width correctly for 2 buttons
//              } // end of else
    }				// end of Atomic clock widget
    {
	// Add the "OK" button
	NxButton *o;		// Button widget
	...
	o = new NxButton(0, 0, _timewin->w(), BUTTON_HEIGHT);
	o->box(FL_BORDER_BOX);
	...
	_timeui = o =
	    new NxTmInput(c_x, c_y, (int) (1.5 * BUTTON_WIDTH), BUTTON_HEIGHT,
			  0, 3);
	//o->color(NxApp::Instance()->getGlobalColor(APP_BG));
	for (int i = 0; i < o->GetUnits(); i++) {
	    ...
	    //o->Units[i]->color(NxApp::Instance()->getGlobalColor(APP_BG));
	    //o->Units[i]->selection_color(NxApp::Instance()->
...
//              up->box(FL_BORDER_BOX);
	...
	_cal = o = new Fl_Calendar(d_x, d_y, (w - (2 * d_x)),
...
//
//      Static fucntions
//              adj_tm_cb(Fl_Widget *, void *)
//              map_click_cb(Fl_Widget *, void *)
//              ipcHandler(int, void *)
//              save_exit_cb(Fl_Widget *, void *)
//              sel_city_cb(Fl_Widget *, void *)
//              set_dt_tm_cb(Fl_Widget *, void *)
//              set_tz_cb(Fl_Widget *, void *)
//              show_sc_win_cb(Fl_Widget *, void *)
//              Sunclock_Timeout(Fl_Widget *, void *)
//              Timer_Timeout(Fl_Widget *, void *)
//              Toggle_tz_cb(Fl_Widget *, void *)
//              upd_tm_cb(Fl_Widget *, void *)
//              
//------------------------------------------------------------------------------
void
