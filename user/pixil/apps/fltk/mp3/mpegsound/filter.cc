...
#define OUT2(v)   out2[(96-(v)-32)*16]=out2[((v)-32)*16]
	...
	p0 = hcos_64[0] * (x[0] - x[31]);
	p1 = hcos_64[1] * (x[1] - x[30]);
	p2 = hcos_64[2] * (x[2] - x[29]);
	p3 = hcos_64[3] * (x[3] - x[28]);
	p4 = hcos_64[4] * (x[4] - x[27]);
	p5 = hcos_64[5] * (x[5] - x[26]);
	p6 = hcos_64[6] * (x[6] - x[25]);
	p7 = hcos_64[7] * (x[7] - x[24]);
	p8 = hcos_64[8] * (x[8] - x[23]);
	p9 = hcos_64[9] * (x[9] - x[22]);
	pa = hcos_64[10] * (x[10] - x[21]);
	pb = hcos_64[11] * (x[11] - x[20]);
	pc = hcos_64[12] * (x[12] - x[19]);
	pd = hcos_64[13] * (x[13] - x[18]);
	pe = hcos_64[14] * (x[14] - x[17]);
	pf = hcos_64[15] * (x[15] - x[16]);
    }
...
#ifdef FIXED_POINT
#define RAW( r )  (r.x>>(FIXED_POINT_FRACBITS-15))
#else
#define RAW( r )  (int)(r*scalefactor)
#endif

#define SAVE \
	raw=RAW(r); \
	if(raw>MAXSCALE)raw=MAXSCALE; \
	if(raw<MINSCALE)raw=MINSCALE; \
	putraw(raw);
#define OS  r=*vp * *dp++
...
#else
    // reduced memory footprint and cache trashing for small cache machines...
    for (; i; i--, vp += 15) {
	OS;
	switch (15 - calcbufferoffset) {
	case 0:
	    OP;
	case 1:
	    OP;
...
#else
    // reduced memory footprint and cache trashing...
    for (; i; i--, vp1 += 15, vp2 += 15) {
	OS;
	switch (15 - calcbufferoffset) {
	case 0:
...
void
Mpegtoraw::subbandsynthesis(REAL * fractionL, REAL * fractionR)
{
