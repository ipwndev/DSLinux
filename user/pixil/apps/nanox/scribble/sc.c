...
*/
	...
	ZZZ('C') {
	    for (i = 0; i < sc->nclasses; i++) {
		scd = sc->classdope[i];
		PrintVector(scd->average, "%5.5s %5g ", scd->name, disc[i]);
	    }
	}
    }
     ...
     register sClassifier sc;
{
	...
	for (j = i + 1; j < NCOLS(d); j++) {
	    d[i][j] = MahalanobisDistance(sc->classdope[i]->average,
	...
	printf("%2d) %10.10s to %10.10s d=%g nstd=%g\n",
	       ...
	       sc->classdope[mj]->name, d[mi][mj], sqrt(d[mi][mj]));
