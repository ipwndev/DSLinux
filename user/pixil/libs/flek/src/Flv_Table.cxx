...
void
Flv_Table::draw_row(int Offset, int &X, int &Y, int &, int &H, int R)
{
    int c, cw, CX, FW;
    int dX, dY, dW, dH;
    int TX, TY, TW, TH;
...
void
Flv_Table::draw_cell(int Offset, int &X, int &Y, int &W, int &H, int R, int C)
{
    Fl_Boxtype bt;
    Flv_Style s;
...
bool
Flv_Table::get_cell_bounds(int &X, int &Y, int &W, int &H, int R, int C)
{
...
Flv_Table::draw(void)
{
	...
	rh = row_height(r);
	if (vlast_row == row()
	    || (vlast_row != row() && (r == vlast_row || r == row()))) {
	    fl_clip(X, Y, W, rh);
	    CX = X;
	    CY = Y;
	    CW = rw;
...
void
Flv_Table::add_selection_style(Flv_Style & s, int R, int C)
{
...
#ifdef PIXIL
	    s.background(NxApp::GlobalColor(HILIGHT));
#else
...
void
Flv_Table::cell_area(int &X, int &Y, int &W, int &H)
{
    client_area(X, Y, W, H);
    if (label() && *label()) {
...
bool
Flv_Table::cell_selected(int R, int C)
{
    return (col_selected(C) && row_selected(R));
}

int
Flv_Table::col_width(int C)	//      Get column width
{
    int fw = vcol_width;
    Flv_Style *cols;
...
int
Flv_Table::col_width(int n, int c)	//      Set column width
{
    int cw = col_width(c);
    if (c < -3)
	c = -3;
...
void
Flv_Table::get_style(Flv_Style & s, int R, int C)
{
...
Flv_Table::handle(int event)
{
	...
	if (x < veditor->x() || y < veditor->y()
	    || x > veditor->x() + veditor->w()
...
int
Flv_Table::internal_handle(int event)
{
...
//                              Fl_Group::handle(event);
	...
	r = 0;
#if FL_MAJOR_VERSION == 1
	if (Fl::event_button1() && (buttons() & FLV_BUTTON1))
	    r = 1;
	if (Fl::event_button2() && (buttons() & FLV_BUTTON2))
	    r = 1;
	if (Fl::event_button3() && (buttons() & FLV_BUTTON3))
	    r = 1;
#else
	if ((Fl::event_state() & FL_BUTTON1) && (buttons() & FLV_BUTTON1))
	    r = 1;
	if ((Fl::event_state() & FL_BUTTON2) && (buttons() & FLV_BUTTON2))
	    r = 1;
	if ((Fl::event_state() & FL_BUTTON3) && (buttons() & FLV_BUTTON3))
	    r = 1;
#endif
...
bool
Flv_Table::col_resizable(int c)	//      Get/set column locked status
{
    Flv_Style *s;
    bool l = true;
...
bool
Flv_Table::col_resizable(bool n, int c)
{
...
int
Flv_Table::select_start_col(int n)
{
    if (n >= vcols)
	n = vcols - 1;
    if (n < 0)
...
Flv_Table::check_cursor(void)
{
	...
	if (!resize) {
		...
		if (!resize) {
		    if (col_footer()) {
			size = col_width(-2);
			resize |= (ex <= X + W && ex >= X + W - size - FUDGE
				   && inh);
		    }
		}
	    }
	...
	for (WW -= row_offset(), v = 0; WW < R && WW < X + W && v < cols();
	     WW += size, v++) {
	    size = col_width(v);
...
void
Flv_Table::start_edit(void)	//      Start editing
{
    if (!vediting) {
	vediting = true;
	switch_editor(row(), col());
...
void
Flv_Table::cancel_edit(void)	//      Cancel editing
{
    if (veditor) {
	veditor->hide();
	veditor->redraw();
...
void
Flv_Table::switch_editor(int nr, int nc)
{
    Flv_Style s;
